// -------------------------------------------------------------
// 
// File Name: D:\ljx\code\GUIDANCE-ljx3\slsf_randgen\slsf\reportsneo\2024-06-06-16-53-16\Verilog_hdlsrc\sampleModel1810\sampleModel1810_5_sub\Subsystem.v
// Created: 2024-08-14 12:16:43
// 
// Generated by MATLAB 9.14 and HDL Coder 4.1
// 
// 
// -- -------------------------------------------------------------
// -- Rate and Clocking Details
// -- -------------------------------------------------------------
// Model base rate: 1
// Target subsystem base rate: 1
// 
// 
// Clock Enable  Sample Time
// -- -------------------------------------------------------------
// ce_out        1
// -- -------------------------------------------------------------
// 
// 
// Output Signal                 Clock Enable  Sample Time
// -- -------------------------------------------------------------
// Hdl_out                       ce_out        1
// -- -------------------------------------------------------------
// 
// -------------------------------------------------------------


// -------------------------------------------------------------
// 
// Module: Subsystem
// Source Path: sampleModel1810_5_sub/Subsystem
// Hierarchy Level: 0
// 
// -------------------------------------------------------------

`timescale 1 ns / 1 ns

module Subsystem
          (clk,
           reset,
           clk_enable,
           ce_out,
           Hdl_out);


  input   clk;
  input   reset;
  input   clk_enable;
  output  ce_out;
  output  [7:0] Hdl_out;  // uint8


  wire enb;
  wire [7:0] cfblk129_out1;  // uint8
  wire [7:0] cfblk72_out1;  // uint8
  wire [7:0] dtc_out;  // ufix8
  wire [7:0] cfblk20_out1;  // uint8
  wire [7:0] cfblk79_const_val_1;  // uint8
  wire [7:0] cfblk79_out1;  // uint8
  wire [7:0] Mysubsystem_33_out1;  // uint8
  wire [7:0] Mysubsystem_33_out2;  // uint8
  wire [7:0] cfblk128_out1;  // uint8
  wire [7:0] cfblk130_out1;  // uint8
  wire [7:0] bitMask_for_cfblk2;  // uint8
  wire [7:0] cfblk133_out1;  // uint8
  wire [7:0] cfblk19_out1;  // uint8
  wire [7:0] cfblk96_const_val_1;  // uint8
  wire [7:0] cfblk131_out1;  // uint8
  wire [7:0] cfblk1_out1;  // uint8
  wire [7:0] cfblk106_const_val_1;  // uint8
  wire [7:0] cfblk106_out1;  // uint8
  wire [7:0] cfblk100_const_val_1;  // uint8
  wire [7:0] cfblk60_const_val_1;  // uint8
  wire [7:0] cfblk3_out1;  // uint8
  wire [7:0] cfblk33_out1;  // uint8
  wire [7:0] cfblk61_out1;  // uint8
  wire [7:0] dtc_out_1;  // ufix8
  wire [7:0] cfblk50_out1;  // uint8
  wire [7:0] cfblk78_out1;  // uint8
  wire [7:0] dtc_out_2;  // ufix8
  wire [7:0] cfblk110_out1;  // uint8
  wire [7:0] cfblk80_out1;  // uint8
  reg [7:0] cfblk148_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk148_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk148_out1;  // uint8
  wire [7:0] cfblk26_out1;  // uint8
  reg [7:0] cfblk141_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk141_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk141_out1;  // uint8
  wire [7:0] cfblk55_out1;  // uint8
  wire [7:0] cfblk77_out1;  // uint8
  wire [7:0] cfblk113_out1;  // uint8
  wire [7:0] cfblk57_out1;  // uint8
  reg [7:0] cfblk150_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk150_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk150_out1;  // uint8
  wire [7:0] cfblk44_out1;  // uint8
  wire [7:0] cfblk54_out1;  // uint8
  wire [7:0] cfblk39_out1;  // uint8
  reg [7:0] cfblk94_out1;  // uint8
  reg [7:0] cfblk142_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk142_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk142_out1;  // uint8
  wire [7:0] cfblk2_out1;  // uint8
  wire [7:0] cfblk114_out1;  // uint8
  wire [7:0] cfblk101_out1;  // uint8
  reg [7:0] cfblk120_out1;  // uint8
  wire [7:0] cfblk24_out1;  // uint8
  wire [7:0] cfblk25_out1;  // uint8
  wire [7:0] cfblk81_out1;  // uint8
  wire [7:0] cfblk100_out1;  // uint8
  reg [7:0] cfblk140_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk140_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk140_out1;  // uint8
  wire [7:0] cfblk84_out1;  // uint8
  wire [7:0] cfblk60_out1;  // uint8
  wire [7:0] cfblk82_out1;  // uint8
  wire [7:0] cfblk135_out1;  // uint8
  wire [7:0] dtc_out_3;  // ufix8
  wire [7:0] cfblk83_out1;  // uint8
  wire [7:0] cfblk18_out1;  // uint8
  reg [7:0] cfblk149_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk149_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk149_out1;  // uint8
  wire [7:0] cfblk36_out1;  // uint8
  wire [7:0] cfblk46_out1;  // uint8
  wire [7:0] cfblk41_out1;  // uint8
  wire [7:0] dtc_out_4;  // ufix8
  wire [7:0] cfblk70_out1;  // uint8
  wire [7:0] cfblk23_out1;  // uint8
  wire [7:0] cfblk96_out1;  // uint8
  wire [7:0] cfblk28_out1;  // uint8
  wire [7:0] dtc_out_5;  // ufix8
  wire [7:0] cfblk64_out1;  // uint8
  wire [7:0] cfblk63_out1;  // uint8
  wire [7:0] cfblk117_out1;  // uint8
  wire [7:0] cfblk122_out1;  // uint8
  wire [7:0] cfblk10_out1;  // uint8
  reg [7:0] cfblk147_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk147_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk147_out1;  // uint8
  wire [7:0] cfblk9_out1;  // uint8
  wire [7:0] dtc_out_6;  // ufix8
  wire [7:0] cfblk11_out1;  // uint8
  wire [7:0] dtc_out_7;  // ufix8
  wire [7:0] cfblk67_out1;  // uint8
  wire [7:0] dtc_out_8;  // ufix8
  wire [7:0] cfblk45_out1;  // uint8
  wire [7:0] cfblk59_out1;  // uint8
  wire [7:0] dtc_out_9;  // ufix8
  wire [7:0] cfblk56_out1;  // uint8
  wire [7:0] cfblk89_out1;  // uint8
  wire [7:0] dtc_out_10;  // ufix8
  wire [7:0] cfblk31_out1;  // uint8
  wire [7:0] cfblk5_out1;  // uint8
  wire [7:0] cfblk98_out1;  // uint8
  wire [7:0] cfblk30_out1;  // uint8
  wire [7:0] cfblk27_out1;  // uint8
  reg [7:0] cfblk74_out1;  // uint8
  wire [7:0] cfblk35_out1;  // uint8
  reg [7:0] cfblk146_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk146_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk146_out1;  // uint8
  wire [7:0] cfblk136_out1;  // uint8
  wire [7:0] cfblk86_out1;  // uint8
  wire [7:0] dtc_out_11;  // ufix8
  wire [7:0] cfblk111_out1;  // uint8
  wire [7:0] cfblk4_out1;  // uint8
  reg [7:0] cfblk143_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk143_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk143_out1;  // uint8
  wire [7:0] cfblk69_out1;  // uint8
  wire [7:0] cfblk16_out1;  // uint8
  wire [7:0] dtc_out_12;  // ufix8
  wire [7:0] cfblk73_out1;  // uint8
  reg [7:0] cfblk65_out1;  // uint8
  wire [7:0] cfblk42_out1;  // uint8
  wire [7:0] cfblk6_out1;  // uint8
  wire [7:0] cfblk124_out1;  // uint8
  wire [7:0] cfblk43_out1;  // uint8
  wire [7:0] cfblk112_out1;  // uint8
  wire [7:0] cfblk108_out1;  // uint8
  wire [7:0] cfblk49_out1;  // uint8
  reg [7:0] cfblk145_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk145_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk145_out1;  // uint8
  wire [7:0] cfblk52_out1;  // uint8
  reg [7:0] cfblk90_out1;  // uint8
  reg [7:0] cfblk144_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk144_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk144_out1;  // uint8
  reg [8:0] cfblk94_div_temp;  // ufix9
  reg [8:0] cfblk94_t_0_0;  // ufix9
  reg [8:0] cfblk120_div_temp;  // ufix9
  reg [8:0] cfblk120_t_0_0;  // ufix9
  reg [8:0] cfblk74_div_temp;  // ufix9
  reg [8:0] cfblk74_t_0_0;  // ufix9
  reg [8:0] cfblk65_div_temp;  // ufix9
  reg [8:0] cfblk65_t_0_0;  // ufix9
  reg [8:0] cfblk90_div_temp;  // ufix9
  reg [8:0] cfblk90_t_0_0;  // ufix9


  cfblk129 u_cfblk129 (.clk(clk),
                       .reset(reset),
                       .enb(clk_enable),
                       .y(cfblk129_out1)  // uint8
                       );

  assign cfblk72_out1 = (cfblk129_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  assign dtc_out = cfblk72_out1;



  assign cfblk20_out1 = dtc_out;



  assign cfblk79_const_val_1 = 8'b00000000;



  assign cfblk79_out1 = cfblk20_out1 + cfblk79_const_val_1;



  Mysubsystem_33 u_Mysubsystem_33 (.clk(clk),
                                   .reset(reset),
                                   .enb(clk_enable),
                                   .Out1(Mysubsystem_33_out1),  // uint8
                                   .Out2(Mysubsystem_33_out2)  // uint8
                                   );

  assign cfblk128_out1 = 8'b00000001;



  assign cfblk130_out1 = 8'b00000001;



  assign bitMask_for_cfblk2 = 8'b11011001;



  assign enb = clk_enable;

  assign cfblk133_out1 = 8'b00000001;



  assign cfblk19_out1 = (cfblk133_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  assign cfblk96_const_val_1 = 8'b00000000;



  assign cfblk131_out1 = 8'b00000000;



  cfblk1 u_cfblk1 (.clk(clk),
                   .reset(reset),
                   .enb(clk_enable),
                   .U(cfblk128_out1),  // uint8
                   .Y(cfblk1_out1)  // uint8
                   );

  assign cfblk106_const_val_1 = 8'b00000000;



  assign cfblk106_out1 = cfblk1_out1 + cfblk106_const_val_1;



  assign cfblk100_const_val_1 = 8'b00000000;



  assign cfblk60_const_val_1 = 8'b00000000;



  cfblk3 u_cfblk3 (.u(cfblk1_out1),  // uint8
                   .y(cfblk3_out1)  // uint8
                   );

  assign cfblk33_out1 = (cfblk3_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  assign dtc_out_1 = cfblk61_out1;



  assign cfblk50_out1 = dtc_out_1;



  assign dtc_out_2 = cfblk78_out1;



  assign cfblk110_out1 = dtc_out_2;



  always @(posedge clk or posedge reset)
    begin : cfblk148_process
      if (reset == 1'b1) begin
        cfblk148_reg[0] <= 8'b00000000;
        cfblk148_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk148_reg[0] <= cfblk148_reg_next[0];
          cfblk148_reg[1] <= cfblk148_reg_next[1];
        end
      end
    end

  assign cfblk148_out1 = cfblk148_reg[1];
  assign cfblk148_reg_next[0] = cfblk80_out1;
  assign cfblk148_reg_next[1] = cfblk148_reg[0];



  always @(posedge clk or posedge reset)
    begin : cfblk141_process
      if (reset == 1'b1) begin
        cfblk141_reg[0] <= 8'b00000000;
        cfblk141_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk141_reg[0] <= cfblk141_reg_next[0];
          cfblk141_reg[1] <= cfblk141_reg_next[1];
        end
      end
    end

  assign cfblk141_out1 = cfblk141_reg[1];
  assign cfblk141_reg_next[0] = cfblk26_out1;
  assign cfblk141_reg_next[1] = cfblk141_reg[0];



  DotProduct u_cfblk113_inst (.in1(cfblk55_out1),  // uint8
                              .in2(cfblk77_out1),  // uint8
                              .out1(cfblk113_out1)  // uint8
                              );

  always @(posedge clk or posedge reset)
    begin : cfblk150_process
      if (reset == 1'b1) begin
        cfblk150_reg[0] <= 8'b00000000;
        cfblk150_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk150_reg[0] <= cfblk150_reg_next[0];
          cfblk150_reg[1] <= cfblk150_reg_next[1];
        end
      end
    end

  assign cfblk150_out1 = cfblk150_reg[1];
  assign cfblk150_reg_next[0] = cfblk57_out1;
  assign cfblk150_reg_next[1] = cfblk150_reg[0];



  assign cfblk44_out1 = cfblk150_out1 - cfblk113_out1;



  DotProduct_block2 u_cfblk54_inst (.in1(cfblk33_out1),  // uint8
                                    .in2(cfblk44_out1),  // uint8
                                    .out1(cfblk54_out1)  // uint8
                                    );

  DotProduct_block4 u_cfblk61_inst (.in1(cfblk54_out1),  // uint8
                                    .in2(cfblk128_out1),  // uint8
                                    .out1(cfblk61_out1)  // uint8
                                    );

  always @(cfblk39_out1, cfblk61_out1) begin
    cfblk94_div_temp = 9'b000000000;
    cfblk94_t_0_0 = 9'b000000000;
    if (cfblk61_out1 == 8'b00000000) begin
      cfblk94_out1 = 8'b11111111;
    end
    else begin
      cfblk94_t_0_0 = {1'b0, cfblk39_out1};
      cfblk94_div_temp = cfblk94_t_0_0 / cfblk61_out1;
      if (cfblk94_div_temp[8] != 1'b0) begin
        cfblk94_out1 = 8'b11111111;
      end
      else begin
        cfblk94_out1 = cfblk94_div_temp[7:0];
      end
    end
  end



  assign cfblk26_out1 = cfblk94_out1 - cfblk44_out1;



  always @(posedge clk or posedge reset)
    begin : cfblk142_process
      if (reset == 1'b1) begin
        cfblk142_reg[0] <= 8'b00000000;
        cfblk142_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk142_reg[0] <= cfblk142_reg_next[0];
          cfblk142_reg[1] <= cfblk142_reg_next[1];
        end
      end
    end

  assign cfblk142_out1 = cfblk142_reg[1];
  assign cfblk142_reg_next[0] = cfblk26_out1;
  assign cfblk142_reg_next[1] = cfblk142_reg[0];



  assign cfblk114_out1 = cfblk129_out1 - cfblk2_out1;



  always @(cfblk101_out1, cfblk114_out1) begin
    cfblk120_div_temp = 9'b000000000;
    cfblk120_t_0_0 = 9'b000000000;
    if (cfblk114_out1 == 8'b00000000) begin
      cfblk120_out1 = 8'b11111111;
    end
    else begin
      cfblk120_t_0_0 = {1'b0, cfblk101_out1};
      cfblk120_div_temp = cfblk120_t_0_0 / cfblk114_out1;
      if (cfblk120_div_temp[8] != 1'b0) begin
        cfblk120_out1 = 8'b11111111;
      end
      else begin
        cfblk120_out1 = cfblk120_div_temp[7:0];
      end
    end
  end



  assign cfblk24_out1 = (cfblk120_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  assign cfblk81_out1 = cfblk24_out1 - cfblk25_out1;



  assign cfblk100_out1 = cfblk81_out1 + cfblk100_const_val_1;



  always @(posedge clk or posedge reset)
    begin : cfblk140_process
      if (reset == 1'b1) begin
        cfblk140_reg[0] <= 8'b00000000;
        cfblk140_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk140_reg[0] <= cfblk140_reg_next[0];
          cfblk140_reg[1] <= cfblk140_reg_next[1];
        end
      end
    end

  assign cfblk140_out1 = cfblk140_reg[1];
  assign cfblk140_reg_next[0] = cfblk100_out1;
  assign cfblk140_reg_next[1] = cfblk140_reg[0];



  DotProduct_block7 u_cfblk84_inst (.in1(cfblk106_out1),  // uint8
                                    .in2(cfblk140_out1),  // uint8
                                    .out1(cfblk84_out1)  // uint8
                                    );

  assign cfblk60_out1 = cfblk84_out1 + cfblk60_const_val_1;



  assign cfblk82_out1 = cfblk39_out1 + cfblk57_out1;



  cfblk135 u_cfblk135 (.In1(cfblk82_out1),  // uint8
                       .Out1(cfblk135_out1)  // uint8
                       );

  assign dtc_out_3 = cfblk39_out1;



  assign cfblk83_out1 = dtc_out_3;



  assign cfblk18_out1 = cfblk83_out1 + cfblk135_out1;



  always @(posedge clk or posedge reset)
    begin : cfblk149_process
      if (reset == 1'b1) begin
        cfblk149_reg[0] <= 8'b00000000;
        cfblk149_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk149_reg[0] <= cfblk149_reg_next[0];
          cfblk149_reg[1] <= cfblk149_reg_next[1];
        end
      end
    end

  assign cfblk149_out1 = cfblk149_reg[1];
  assign cfblk149_reg_next[0] = cfblk18_out1;
  assign cfblk149_reg_next[1] = cfblk149_reg[0];



  assign cfblk36_out1 = cfblk149_out1 - cfblk60_out1;



  DotProduct_block1 u_cfblk46_inst (.in1(cfblk36_out1),  // uint8
                                    .in2(cfblk142_out1),  // uint8
                                    .out1(cfblk46_out1)  // uint8
                                    );

  assign dtc_out_4 = cfblk41_out1;



  assign cfblk77_out1 = dtc_out_4;



  assign cfblk23_out1 = cfblk70_out1 - cfblk77_out1;



  assign cfblk96_out1 = cfblk23_out1 + cfblk96_const_val_1;



  assign cfblk28_out1 = cfblk19_out1 + cfblk39_out1;



  assign cfblk57_out1 = (cfblk28_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  assign dtc_out_5 = cfblk57_out1;



  assign cfblk64_out1 = dtc_out_5;



  assign cfblk63_out1 = cfblk64_out1 + cfblk96_out1;



  assign cfblk122_out1 = (cfblk117_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  assign cfblk2_out1 = cfblk122_out1 & bitMask_for_cfblk2;



  assign cfblk10_out1 = cfblk130_out1 + cfblk2_out1;



  always @(posedge clk or posedge reset)
    begin : cfblk147_process
      if (reset == 1'b1) begin
        cfblk147_reg[0] <= 8'b00000000;
        cfblk147_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk147_reg[0] <= cfblk147_reg_next[0];
          cfblk147_reg[1] <= cfblk147_reg_next[1];
        end
      end
    end

  assign cfblk147_out1 = cfblk147_reg[1];
  assign cfblk147_reg_next[0] = cfblk10_out1;
  assign cfblk147_reg_next[1] = cfblk147_reg[0];



  assign dtc_out_6 = cfblk9_out1;



  assign cfblk11_out1 = dtc_out_6;



  assign dtc_out_7 = cfblk11_out1;



  assign cfblk67_out1 = dtc_out_7;



  assign cfblk55_out1 = cfblk67_out1 + cfblk147_out1;



  DotProduct_block u_cfblk41_inst (.in1(cfblk128_out1),  // uint8
                                   .in2(cfblk55_out1),  // uint8
                                   .out1(cfblk41_out1)  // uint8
                                   );

  assign dtc_out_8 = cfblk80_out1;



  assign cfblk45_out1 = dtc_out_8;



  DotProduct_block3 u_cfblk59_inst (.in1(cfblk45_out1),  // uint8
                                    .in2(cfblk41_out1),  // uint8
                                    .out1(cfblk59_out1)  // uint8
                                    );

  assign dtc_out_9 = cfblk59_out1;



  assign cfblk56_out1 = dtc_out_9;



  DotProduct_block8 u_cfblk89_inst (.in1(Mysubsystem_33_out2),  // uint8
                                    .in2(cfblk56_out1),  // uint8
                                    .out1(cfblk89_out1)  // uint8
                                    );

  assign dtc_out_10 = cfblk89_out1;



  assign cfblk31_out1 = dtc_out_10;



  assign cfblk101_out1 = cfblk31_out1 - cfblk20_out1;



  assign cfblk117_out1 = cfblk80_out1 - cfblk5_out1;



  DotProduct_block6 u_cfblk78_inst (.in1(cfblk117_out1),  // uint8
                                    .in2(cfblk101_out1),  // uint8
                                    .out1(cfblk78_out1)  // uint8
                                    );

  DotProduct_block9 u_cfblk98_inst (.in1(cfblk78_out1),  // uint8
                                    .in2(cfblk89_out1),  // uint8
                                    .out1(cfblk98_out1)  // uint8
                                    );

  assign cfblk30_out1 = cfblk98_out1 + Mysubsystem_33_out1;



  assign cfblk27_out1 = cfblk30_out1 + cfblk63_out1;



  assign cfblk35_out1 = cfblk27_out1 - cfblk74_out1;



  always @(posedge clk or posedge reset)
    begin : cfblk146_process
      if (reset == 1'b1) begin
        cfblk146_reg[0] <= 8'b00000000;
        cfblk146_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk146_reg[0] <= cfblk146_reg_next[0];
          cfblk146_reg[1] <= cfblk146_reg_next[1];
        end
      end
    end

  assign cfblk146_out1 = cfblk146_reg[1];
  assign cfblk146_reg_next[0] = cfblk35_out1;
  assign cfblk146_reg_next[1] = cfblk146_reg[0];



  assign cfblk80_out1 = cfblk39_out1 + cfblk146_out1;



  cfblk136 u_cfblk136 (.In1(cfblk80_out1),  // uint8
                       .Out1(cfblk136_out1)  // uint8
                       );

  always @(cfblk136_out1, cfblk96_out1) begin
    cfblk74_div_temp = 9'b000000000;
    cfblk74_t_0_0 = 9'b000000000;
    if (cfblk96_out1 == 8'b00000000) begin
      cfblk74_out1 = 8'b11111111;
    end
    else begin
      cfblk74_t_0_0 = {1'b0, cfblk136_out1};
      cfblk74_div_temp = cfblk74_t_0_0 / cfblk96_out1;
      if (cfblk74_div_temp[8] != 1'b0) begin
        cfblk74_out1 = 8'b11111111;
      end
      else begin
        cfblk74_out1 = cfblk74_div_temp[7:0];
      end
    end
  end



  assign cfblk25_out1 = cfblk74_out1 + cfblk131_out1;



  assign cfblk86_out1 = cfblk79_out1 + cfblk25_out1;



  assign dtc_out_11 = cfblk86_out1;



  assign cfblk111_out1 = dtc_out_11;



  cfblk4 u_cfblk4 (.clk(clk),
                   .reset(reset),
                   .enb(clk_enable),
                   .U(cfblk111_out1),  // uint8
                   .Y(cfblk4_out1)  // uint8
                   );

  assign cfblk5_out1 = cfblk4_out1 & 8'b11111110;



  always @(posedge clk or posedge reset)
    begin : cfblk143_process
      if (reset == 1'b1) begin
        cfblk143_reg[0] <= 8'b00000000;
        cfblk143_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk143_reg[0] <= cfblk143_reg_next[0];
          cfblk143_reg[1] <= cfblk143_reg_next[1];
        end
      end
    end

  assign cfblk143_out1 = cfblk143_reg[1];
  assign cfblk143_reg_next[0] = cfblk5_out1;
  assign cfblk143_reg_next[1] = cfblk143_reg[0];



  assign cfblk69_out1 = cfblk143_out1 + cfblk46_out1;



  DotProduct_block5 u_cfblk70_inst (.in1(cfblk69_out1),  // uint8
                                    .in2(cfblk141_out1),  // uint8
                                    .out1(cfblk70_out1)  // uint8
                                    );

  assign cfblk16_out1 = cfblk70_out1 + cfblk148_out1;



  assign dtc_out_12 = cfblk16_out1;



  assign cfblk9_out1 = dtc_out_12;



  assign cfblk73_out1 = cfblk9_out1 + cfblk4_out1;



  always @(cfblk110_out1, cfblk73_out1) begin
    cfblk65_div_temp = 9'b000000000;
    cfblk65_t_0_0 = 9'b000000000;
    if (cfblk110_out1 == 8'b00000000) begin
      cfblk65_out1 = 8'b11111111;
    end
    else begin
      cfblk65_t_0_0 = {1'b0, cfblk73_out1};
      cfblk65_div_temp = cfblk65_t_0_0 / cfblk110_out1;
      if (cfblk65_div_temp[8] != 1'b0) begin
        cfblk65_out1 = 8'b11111111;
      end
      else begin
        cfblk65_out1 = cfblk65_div_temp[7:0];
      end
    end
  end



  assign cfblk42_out1 = cfblk65_out1 + cfblk41_out1;



  assign cfblk6_out1 = cfblk42_out1 | 8'b00000001;



  assign cfblk124_out1 = (cfblk6_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  assign cfblk43_out1 = cfblk124_out1 + cfblk129_out1;



  assign cfblk108_out1 = cfblk112_out1 - cfblk43_out1;



  assign cfblk49_out1 = cfblk108_out1 + cfblk129_out1;



  always @(posedge clk or posedge reset)
    begin : cfblk145_process
      if (reset == 1'b1) begin
        cfblk145_reg[0] <= 8'b00000000;
        cfblk145_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk145_reg[0] <= cfblk145_reg_next[0];
          cfblk145_reg[1] <= cfblk145_reg_next[1];
        end
      end
    end

  assign cfblk145_out1 = cfblk145_reg[1];
  assign cfblk145_reg_next[0] = cfblk49_out1;
  assign cfblk145_reg_next[1] = cfblk145_reg[0];



  assign cfblk52_out1 = cfblk145_out1 - cfblk43_out1;



  assign cfblk112_out1 = cfblk52_out1 + cfblk27_out1;



  always @(cfblk112_out1, cfblk30_out1) begin
    cfblk90_div_temp = 9'b000000000;
    cfblk90_t_0_0 = 9'b000000000;
    if (cfblk30_out1 == 8'b00000000) begin
      cfblk90_out1 = 8'b11111111;
    end
    else begin
      cfblk90_t_0_0 = {1'b0, cfblk112_out1};
      cfblk90_div_temp = cfblk90_t_0_0 / cfblk30_out1;
      if (cfblk90_div_temp[8] != 1'b0) begin
        cfblk90_out1 = 8'b11111111;
      end
      else begin
        cfblk90_out1 = cfblk90_div_temp[7:0];
      end
    end
  end



  always @(posedge clk or posedge reset)
    begin : cfblk144_process
      if (reset == 1'b1) begin
        cfblk144_reg[0] <= 8'b00000000;
        cfblk144_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk144_reg[0] <= cfblk144_reg_next[0];
          cfblk144_reg[1] <= cfblk144_reg_next[1];
        end
      end
    end

  assign cfblk144_out1 = cfblk144_reg[1];
  assign cfblk144_reg_next[0] = cfblk90_out1;
  assign cfblk144_reg_next[1] = cfblk144_reg[0];



  assign cfblk39_out1 = cfblk144_out1 + cfblk50_out1;



  assign Hdl_out = cfblk39_out1;

  assign ce_out = clk_enable;

endmodule  // Subsystem

