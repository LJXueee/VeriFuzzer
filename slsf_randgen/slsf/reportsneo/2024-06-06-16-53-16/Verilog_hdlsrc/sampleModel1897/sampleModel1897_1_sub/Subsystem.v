// -------------------------------------------------------------
// 
// File Name: D:\ljx\code\GUIDANCE-ljx3\slsf_randgen\slsf\reportsneo\2024-06-06-16-53-16\Verilog_hdlsrc\sampleModel1897\sampleModel1897_1_sub\Subsystem.v
// Created: 2024-07-02 10:41:16
// 
// Generated by MATLAB 9.14 and HDL Coder 4.1
// 
// 
// -- -------------------------------------------------------------
// -- Rate and Clocking Details
// -- -------------------------------------------------------------
// Model base rate: 1
// Target subsystem base rate: 1
// 
// 
// Clock Enable  Sample Time
// -- -------------------------------------------------------------
// ce_out        1
// -- -------------------------------------------------------------
// 
// 
// Output Signal                 Clock Enable  Sample Time
// -- -------------------------------------------------------------
// cfblk122                      ce_out        1
// cfblk124                      ce_out        1
// Hdl_out                       ce_out        1
// -- -------------------------------------------------------------
// 
// -------------------------------------------------------------


// -------------------------------------------------------------
// 
// Module: Subsystem
// Source Path: sampleModel1897_1_sub/Subsystem
// Hierarchy Level: 0
// 
// -------------------------------------------------------------

`timescale 1 ns / 1 ns

module Subsystem
          (clk,
           reset,
           clk_enable,
           ce_out,
           cfblk122,
           cfblk124,
           Hdl_out);


  input   clk;
  input   reset;
  input   clk_enable;
  output  ce_out;
  output  [7:0] cfblk122;  // uint8
  output  [7:0] cfblk124;  // uint8
  output  [7:0] Hdl_out;  // uint8


  wire enb;
  wire [7:0] cfblk126_out1;  // uint8
  wire [7:0] dtc_out;  // ufix8
  wire [7:0] cfblk94_out1;  // uint8
  wire [7:0] cfblk128_out1;  // uint8
  wire [7:0] cfblk35_out1;  // uint8
  wire [7:0] cfblk67_const_val_1;  // uint8
  wire [15:0] Mysubsystem_17_out1;  // uint16
  wire [7:0] Mysubsystem_17_out2;  // uint8
  wire [15:0] Mysubsystem_17_out3;  // uint16
  wire [7:0] Mysubsystem_31_out1;  // uint8
  wire [7:0] cfblk39_const_val_1;  // uint8
  wire [31:0] Mysubsystem_23_out1;  // ufix32_En15
  wire [15:0] Mysubsystem_23_out2;  // uint16
  wire [7:0] cfblk41_const_val_1;  // uint8
  wire [7:0] cfblk51_const_val_1;  // uint8
  wire [7:0] cfblk125_out1;  // uint8
  wire [7:0] cfblk87_const_val_1;  // uint8
  wire [7:0] cfblk48_const_val_1;  // uint8
  wire [7:0] cfblk46_const_val_1;  // uint8
  wire [7:0] cfblk119_const_val_1;  // uint8
  wire [7:0] cfblk45_const_val_1;  // uint8
  wire [7:0] cfblk111_const_val_1;  // uint8
  wire [7:0] cfblk10_const_val_1;  // uint8
  wire [7:0] cfblk13_out1;  // uint8
  wire [7:0] cfblk36_out1;  // uint8
  wire [7:0] cfblk30_out1;  // uint8
  wire [7:0] cfblk61_out1;  // uint8
  reg [7:0] cfblk144_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk144_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk144_out1;  // uint8
  wire [7:0] cfblk102_out1;  // uint8
  reg [7:0] cfblk137_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk137_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk137_out1;  // uint8
  wire [7:0] cfblk76_out1;  // uint8
  reg [7:0] cfblk148_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk148_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk148_out1;  // uint8
  wire [7:0] cfblk119_out1;  // uint8
  reg [7:0] cfblk151_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk151_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk151_out1;  // uint8
  wire [7:0] cfblk8_out1;  // uint8
  reg [7:0] cfblk146_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk146_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk146_out1;  // uint8
  wire [7:0] cfblk78_out1;  // uint8
  reg [7:0] cfblk147_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk147_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk147_out1;  // uint8
  wire [7:0] cfblk2_out1;  // uint8
  wire [7:0] cfblk34_out1;  // uint8
  reg [7:0] cfblk142_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk142_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk142_out1;  // uint8
  reg [7:0] cfblk57_out1;  // uint8
  wire [7:0] cfblk64_out1;  // uint8
  wire cfblk4_relop1;
  wire [31:0] cfblk82_add_temp;  // ufix32
  wire [31:0] cfblk82_1;  // ufix32
  wire [31:0] cfblk82_2;  // ufix32
  wire [7:0] cfblk82_out1;  // uint8
  wire [7:0] cfblk87_out1;  // uint8
  reg [7:0] cfblk149_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk149_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk149_out1;  // uint8
  wire [7:0] dtc_out_1;  // ufix8
  wire [7:0] cfblk96_out1;  // uint8
  wire [7:0] cfblk131_out1;  // uint8
  wire [7:0] dtc_out_2;  // ufix8
  wire [7:0] cfblk95_out1;  // uint8
  wire [7:0] cfblk11_out1;  // uint8
  wire [7:0] cfblk103_out1;  // uint8
  wire [7:0] cfblk5_out1;  // uint8
  reg [7:0] cfblk138_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk138_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk138_out1;  // uint8
  reg [7:0] cfblk100_out1;  // uint8
  wire [7:0] cfblk51_out1;  // uint8
  wire [7:0] cfblk23_out1;  // uint8
  wire [7:0] cfblk111_out1;  // uint8
  wire [7:0] dtc_out_3;  // ufix8
  wire [7:0] cfblk15_out1;  // uint8
  wire [7:0] cfblk50_out1;  // uint8
  wire [7:0] cfblk41_out1;  // uint8
  wire [7:0] cfblk106_out1;  // uint8
  wire [7:0] cfblk98_out1;  // uint8
  reg [7:0] cfblk141_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk141_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk141_out1;  // uint8
  wire [7:0] cfblk116_out1;  // uint8
  wire [7:0] dtc_out_4;  // ufix8
  wire [7:0] cfblk73_out1;  // uint8
  wire [31:0] cfblk75_add_cast;  // ufix32_En14
  wire [31:0] cfblk75_add_cast_1;  // ufix32_En14
  wire [31:0] cfblk75_add_temp;  // ufix32_En14
  wire [7:0] cfblk75_out1;  // uint8
  wire [7:0] cfblk29_out1;  // uint8
  wire [7:0] cfblk10_out1;  // uint8
  wire [7:0] cfblk81_out1;  // uint8
  wire [31:0] cfblk102_add_temp;  // ufix32
  wire [31:0] cfblk102_1;  // ufix32
  wire [31:0] cfblk102_2;  // ufix32
  reg [7:0] cfblk136_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk136_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk136_out1;  // uint8
  wire [7:0] cfblk71_out1;  // uint8
  wire [7:0] cfblk48_out1;  // uint8
  wire [7:0] cfblk46_out1;  // uint8
  wire [7:0] cfblk67_out1;  // uint8
  wire [7:0] cfblk39_out1;  // uint8
  wire [7:0] cfblk6_out1;  // uint8
  wire [7:0] cfblk83_out1;  // uint8
  wire [7:0] cfblk84_out1;  // uint8
  reg [7:0] cfblk139_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk139_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk139_out1;  // uint8
  wire [7:0] cfblk54_out1;  // uint8
  wire [7:0] cfblk3_out1;  // uint8
  wire [7:0] cfblk74_out1;  // uint8
  wire [7:0] cfblk109_out1;  // uint8
  wire [7:0] dtc_out_5;  // ufix8
  wire [7:0] cfblk26_out1;  // uint8
  reg [7:0] cfblk115_out1;  // uint8
  wire [7:0] cfblk68_out1;  // uint8
  wire [7:0] dtc_out_6;  // ufix8
  wire [7:0] cfblk63_out1;  // uint8
  wire [7:0] cfblk47_out1;  // uint8
  wire [7:0] cfblk86_out1;  // uint8
  reg [7:0] cfblk140_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk140_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk140_out1;  // uint8
  wire [7:0] cfblk45_out1;  // uint8
  wire [7:0] cfblk90_out1;  // uint8
  wire [7:0] cfblk59_out1;  // uint8
  wire [7:0] cfblk53_out1;  // uint8
  reg [7:0] cfblk143_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk143_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk143_out1;  // uint8
  wire [7:0] cfblk130_out1;  // uint8
  wire [7:0] cfblk22_const_val_1;  // uint8
  wire [7:0] cfblk22_out1;  // uint8
  wire [7:0] cfblk114_out1;  // uint8
  wire [7:0] cfblk104_out1;  // uint8
  wire [7:0] cfblk79_out1;  // uint8
  wire [7:0] cfblk105_out1;  // uint8
  wire [7:0] dtc_out_7;  // ufix8
  wire [7:0] cfblk56_out1;  // uint8
  wire [7:0] cfblk80_out1;  // uint8
  reg [7:0] cfblk62_out1;  // uint8
  wire [7:0] cfblk72_out1;  // uint8
  wire [7:0] Mysubsystem_5_out1;  // uint8
  reg [8:0] cfblk57_div_temp;  // ufix9
  reg [8:0] cfblk57_t_0_0;  // ufix9
  reg [8:0] cfblk100_div_temp;  // ufix9
  reg [8:0] cfblk100_t_0_0;  // ufix9
  reg [8:0] cfblk115_div_temp;  // ufix9
  reg [8:0] cfblk115_t_0_0;  // ufix9
  reg [8:0] cfblk62_div_temp;  // ufix9
  reg [8:0] cfblk62_t_0_0;  // ufix9


  assign cfblk126_out1 = 8'b00000001;



  assign dtc_out = cfblk126_out1;



  assign cfblk94_out1 = dtc_out;



  assign cfblk128_out1 = 8'b00000001;



  assign cfblk35_out1 = cfblk94_out1 + cfblk128_out1;



  assign cfblk67_const_val_1 = 8'b00000000;



  assign enb = clk_enable;

  Mysubsystem_17 u_Mysubsystem_17 (.clk(clk),
                                   .reset(reset),
                                   .enb(clk_enable),
                                   .y(Mysubsystem_17_out1),  // uint16
                                   .Out2(Mysubsystem_17_out2),  // uint8
                                   .Out3(Mysubsystem_17_out3)  // uint16
                                   );

  Mysubsystem_31 u_Mysubsystem_31 (.In1(Mysubsystem_17_out1),  // uint16
                                   .Out1(Mysubsystem_31_out1)  // uint8
                                   );

  assign cfblk39_const_val_1 = 8'b00000000;



  Mysubsystem_23 u_Mysubsystem_23 (.In1(Mysubsystem_17_out3),  // uint16
                                   .Out1(Mysubsystem_23_out1),  // ufix32_En15
                                   .Out2(Mysubsystem_23_out2)  // uint16
                                   );

  assign cfblk41_const_val_1 = 8'b00000000;



  assign cfblk51_const_val_1 = 8'b00000000;



  cfblk125 u_cfblk125 (.clk(clk),
                       .reset(reset),
                       .enb(clk_enable),
                       .y(cfblk125_out1)  // uint8
                       );

  assign cfblk87_const_val_1 = 8'b00000000;



  assign cfblk48_const_val_1 = 8'b00000000;



  assign cfblk46_const_val_1 = 8'b00000000;



  assign cfblk119_const_val_1 = 8'b00000000;



  assign cfblk45_const_val_1 = 8'b00000000;



  assign cfblk111_const_val_1 = 8'b00000000;



  assign cfblk10_const_val_1 = 8'b00000000;



  assign cfblk30_out1 = cfblk13_out1 + cfblk36_out1;



  always @(posedge clk or posedge reset)
    begin : cfblk144_process
      if (reset == 1'b1) begin
        cfblk144_reg[0] <= 8'b00000000;
        cfblk144_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk144_reg[0] <= cfblk144_reg_next[0];
          cfblk144_reg[1] <= cfblk144_reg_next[1];
        end
      end
    end

  assign cfblk144_out1 = cfblk144_reg[1];
  assign cfblk144_reg_next[0] = cfblk61_out1;
  assign cfblk144_reg_next[1] = cfblk144_reg[0];



  always @(posedge clk or posedge reset)
    begin : cfblk137_process
      if (reset == 1'b1) begin
        cfblk137_reg[0] <= 8'b00000000;
        cfblk137_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk137_reg[0] <= cfblk137_reg_next[0];
          cfblk137_reg[1] <= cfblk137_reg_next[1];
        end
      end
    end

  assign cfblk137_out1 = cfblk137_reg[1];
  assign cfblk137_reg_next[0] = cfblk102_out1;
  assign cfblk137_reg_next[1] = cfblk137_reg[0];



  always @(posedge clk or posedge reset)
    begin : cfblk148_process
      if (reset == 1'b1) begin
        cfblk148_reg[0] <= 8'b00000000;
        cfblk148_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk148_reg[0] <= cfblk148_reg_next[0];
          cfblk148_reg[1] <= cfblk148_reg_next[1];
        end
      end
    end

  assign cfblk148_out1 = cfblk148_reg[1];
  assign cfblk148_reg_next[0] = cfblk76_out1;
  assign cfblk148_reg_next[1] = cfblk148_reg[0];



  always @(posedge clk or posedge reset)
    begin : cfblk151_process
      if (reset == 1'b1) begin
        cfblk151_reg[0] <= 8'b00000000;
        cfblk151_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk151_reg[0] <= cfblk151_reg_next[0];
          cfblk151_reg[1] <= cfblk151_reg_next[1];
        end
      end
    end

  assign cfblk151_out1 = cfblk151_reg[1];
  assign cfblk151_reg_next[0] = cfblk119_out1;
  assign cfblk151_reg_next[1] = cfblk151_reg[0];



  always @(posedge clk or posedge reset)
    begin : cfblk146_process
      if (reset == 1'b1) begin
        cfblk146_reg[0] <= 8'b00000000;
        cfblk146_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk146_reg[0] <= cfblk146_reg_next[0];
          cfblk146_reg[1] <= cfblk146_reg_next[1];
        end
      end
    end

  assign cfblk146_out1 = cfblk146_reg[1];
  assign cfblk146_reg_next[0] = cfblk8_out1;
  assign cfblk146_reg_next[1] = cfblk146_reg[0];



  always @(posedge clk or posedge reset)
    begin : cfblk147_process
      if (reset == 1'b1) begin
        cfblk147_reg[0] <= 8'b00000000;
        cfblk147_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk147_reg[0] <= cfblk147_reg_next[0];
          cfblk147_reg[1] <= cfblk147_reg_next[1];
        end
      end
    end

  assign cfblk147_out1 = cfblk147_reg[1];
  assign cfblk147_reg_next[0] = cfblk78_out1;
  assign cfblk147_reg_next[1] = cfblk147_reg[0];



  cfblk2 u_cfblk2 (.clk(clk),
                   .reset(reset),
                   .enb(clk_enable),
                   .U(cfblk147_out1),  // uint8
                   .Y(cfblk2_out1)  // uint8
                   );

  always @(posedge clk or posedge reset)
    begin : cfblk142_process
      if (reset == 1'b1) begin
        cfblk142_reg[0] <= 8'b00000000;
        cfblk142_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk142_reg[0] <= cfblk142_reg_next[0];
          cfblk142_reg[1] <= cfblk142_reg_next[1];
        end
      end
    end

  assign cfblk142_out1 = cfblk142_reg[1];
  assign cfblk142_reg_next[0] = cfblk34_out1;
  assign cfblk142_reg_next[1] = cfblk142_reg[0];



  always @(cfblk142_out1, cfblk2_out1) begin
    cfblk57_div_temp = 9'b000000000;
    cfblk57_t_0_0 = 9'b000000000;
    if (cfblk2_out1 == 8'b00000000) begin
      cfblk57_out1 = 8'b11111111;
    end
    else begin
      cfblk57_t_0_0 = {1'b0, cfblk142_out1};
      cfblk57_div_temp = cfblk57_t_0_0 / cfblk2_out1;
      if (cfblk57_div_temp[8] != 1'b0) begin
        cfblk57_out1 = 8'b11111111;
      end
      else begin
        cfblk57_out1 = cfblk57_div_temp[7:0];
      end
    end
  end



  assign cfblk64_out1 = cfblk57_out1 + cfblk146_out1;



  assign cfblk4_relop1 = cfblk64_out1 <= cfblk151_out1;



  assign cfblk82_1 = {31'b0, cfblk4_relop1};
  assign cfblk82_2 = {24'b0, cfblk148_out1};
  assign cfblk82_add_temp = cfblk82_1 + cfblk82_2;
  assign cfblk82_out1 = cfblk82_add_temp[7:0];



  assign cfblk87_out1 = cfblk82_out1 + cfblk87_const_val_1;



  always @(posedge clk or posedge reset)
    begin : cfblk149_process
      if (reset == 1'b1) begin
        cfblk149_reg[0] <= 8'b00000000;
        cfblk149_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk149_reg[0] <= cfblk149_reg_next[0];
          cfblk149_reg[1] <= cfblk149_reg_next[1];
        end
      end
    end

  assign cfblk149_out1 = cfblk149_reg[1];
  assign cfblk149_reg_next[0] = cfblk76_out1;
  assign cfblk149_reg_next[1] = cfblk149_reg[0];



  assign dtc_out_1 = cfblk36_out1;



  assign cfblk96_out1 = dtc_out_1;



  cfblk131 u_cfblk131 (.In1(cfblk96_out1),  // uint8
                       .Out1(cfblk131_out1)  // uint8
                       );

  assign dtc_out_2 = cfblk131_out1;



  assign cfblk95_out1 = dtc_out_2;



  assign cfblk103_out1 = (cfblk11_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  assign cfblk5_out1 = cfblk103_out1 & 8'b11111110;



  always @(posedge clk or posedge reset)
    begin : cfblk138_process
      if (reset == 1'b1) begin
        cfblk138_reg[0] <= 8'b00000000;
        cfblk138_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk138_reg[0] <= cfblk138_reg_next[0];
          cfblk138_reg[1] <= cfblk138_reg_next[1];
        end
      end
    end

  assign cfblk138_out1 = cfblk138_reg[1];
  assign cfblk138_reg_next[0] = cfblk5_out1;
  assign cfblk138_reg_next[1] = cfblk138_reg[0];



  assign cfblk11_out1 = cfblk138_out1 + cfblk95_out1;



  always @(cfblk119_out1, cfblk11_out1) begin
    cfblk100_div_temp = 9'b000000000;
    cfblk100_t_0_0 = 9'b000000000;
    if (cfblk11_out1 == 8'b00000000) begin
      cfblk100_out1 = 8'b11111111;
    end
    else begin
      cfblk100_t_0_0 = {1'b0, cfblk119_out1};
      cfblk100_div_temp = cfblk100_t_0_0 / cfblk11_out1;
      if (cfblk100_div_temp[8] != 1'b0) begin
        cfblk100_out1 = 8'b11111111;
      end
      else begin
        cfblk100_out1 = cfblk100_div_temp[7:0];
      end
    end
  end



  assign cfblk51_out1 = cfblk100_out1 + cfblk51_const_val_1;



  assign cfblk23_out1 = cfblk51_out1 + cfblk149_out1;



  assign dtc_out_3 = cfblk111_out1;



  assign cfblk8_out1 = dtc_out_3;



  DotProduct_block3 u_cfblk50_inst (.in1(cfblk15_out1),  // uint8
                                    .in2(cfblk8_out1),  // uint8
                                    .out1(cfblk50_out1)  // uint8
                                    );

  assign cfblk41_out1 = cfblk50_out1 + cfblk41_const_val_1;



  assign cfblk106_out1 = (cfblk41_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  assign cfblk98_out1 = (cfblk106_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  always @(posedge clk or posedge reset)
    begin : cfblk141_process
      if (reset == 1'b1) begin
        cfblk141_reg[0] <= 8'b00000000;
        cfblk141_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk141_reg[0] <= cfblk141_reg_next[0];
          cfblk141_reg[1] <= cfblk141_reg_next[1];
        end
      end
    end

  assign cfblk141_out1 = cfblk141_reg[1];
  assign cfblk141_reg_next[0] = cfblk98_out1;
  assign cfblk141_reg_next[1] = cfblk141_reg[0];



  DotProduct_block1 u_cfblk116_inst (.in1(cfblk141_out1),  // uint8
                                     .in2(cfblk23_out1),  // uint8
                                     .out1(cfblk116_out1)  // uint8
                                     );

  assign dtc_out_4 = cfblk116_out1;



  assign cfblk73_out1 = dtc_out_4;



  assign cfblk75_add_cast = {1'b0, Mysubsystem_23_out1[31:1]};
  assign cfblk75_add_cast_1 = {10'b0, {cfblk73_out1, 14'b00000000000000}};
  assign cfblk75_add_temp = cfblk75_add_cast + cfblk75_add_cast_1;
  assign cfblk75_out1 = cfblk75_add_temp[21:14];



  DotProduct_block5 u_cfblk76_inst (.in1(cfblk75_out1),  // uint8
                                    .in2(Mysubsystem_17_out2),  // uint8
                                    .out1(cfblk76_out1)  // uint8
                                    );

  assign cfblk29_out1 = cfblk76_out1 + cfblk11_out1;



  assign cfblk78_out1 = cfblk29_out1 + cfblk125_out1;



  assign cfblk34_out1 = cfblk10_out1 + cfblk78_out1;



  assign cfblk81_out1 = (cfblk34_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  assign cfblk102_1 = {16'b0, Mysubsystem_23_out2};
  assign cfblk102_2 = {24'b0, cfblk81_out1};
  assign cfblk102_add_temp = cfblk102_1 + cfblk102_2;
  assign cfblk102_out1 = cfblk102_add_temp[7:0];



  always @(posedge clk or posedge reset)
    begin : cfblk136_process
      if (reset == 1'b1) begin
        cfblk136_reg[0] <= 8'b00000000;
        cfblk136_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk136_reg[0] <= cfblk136_reg_next[0];
          cfblk136_reg[1] <= cfblk136_reg_next[1];
        end
      end
    end

  assign cfblk136_out1 = cfblk136_reg[1];
  assign cfblk136_reg_next[0] = cfblk102_out1;
  assign cfblk136_reg_next[1] = cfblk136_reg[0];



  assign cfblk71_out1 = cfblk136_out1 - cfblk87_out1;



  assign cfblk48_out1 = cfblk71_out1 + cfblk48_const_val_1;



  assign cfblk46_out1 = cfblk48_out1 + cfblk46_const_val_1;



  assign cfblk39_out1 = cfblk67_out1 + cfblk39_const_val_1;



  cfblk6 u_cfblk6 (.u(cfblk39_out1),  // uint8
                   .y(cfblk6_out1)  // uint8
                   );

  assign cfblk83_out1 = Mysubsystem_31_out1 + cfblk6_out1;



  DotProduct_block6 u_cfblk84_inst (.in1(cfblk83_out1),  // uint8
                                    .in2(Mysubsystem_17_out2),  // uint8
                                    .out1(cfblk84_out1)  // uint8
                                    );

  always @(posedge clk or posedge reset)
    begin : cfblk139_process
      if (reset == 1'b1) begin
        cfblk139_reg[0] <= 8'b00000000;
        cfblk139_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk139_reg[0] <= cfblk139_reg_next[0];
          cfblk139_reg[1] <= cfblk139_reg_next[1];
        end
      end
    end

  assign cfblk139_out1 = cfblk139_reg[1];
  assign cfblk139_reg_next[0] = cfblk84_out1;
  assign cfblk139_reg_next[1] = cfblk139_reg[0];



  assign cfblk54_out1 = cfblk139_out1 + cfblk46_out1;



  cfblk3 u_cfblk3 (.clk(clk),
                   .reset(reset),
                   .enb(clk_enable),
                   .U(cfblk54_out1),  // uint8
                   .Y(cfblk3_out1)  // uint8
                   );

  assign cfblk74_out1 = cfblk3_out1 + cfblk137_out1;



  assign cfblk109_out1 = cfblk74_out1 + cfblk144_out1;



  assign dtc_out_5 = cfblk109_out1;



  assign cfblk36_out1 = dtc_out_5;



  DotProduct_block2 u_cfblk26_inst (.in1(cfblk35_out1),  // uint8
                                    .in2(cfblk36_out1),  // uint8
                                    .out1(cfblk26_out1)  // uint8
                                    );

  assign cfblk119_out1 = cfblk26_out1 + cfblk119_const_val_1;



  always @(cfblk119_out1, cfblk30_out1) begin
    cfblk115_div_temp = 9'b000000000;
    cfblk115_t_0_0 = 9'b000000000;
    if (cfblk30_out1 == 8'b00000000) begin
      cfblk115_out1 = 8'b11111111;
    end
    else begin
      cfblk115_t_0_0 = {1'b0, cfblk119_out1};
      cfblk115_div_temp = cfblk115_t_0_0 / cfblk30_out1;
      if (cfblk115_div_temp[8] != 1'b0) begin
        cfblk115_out1 = 8'b11111111;
      end
      else begin
        cfblk115_out1 = cfblk115_div_temp[7:0];
      end
    end
  end



  assign dtc_out_6 = cfblk68_out1;



  assign cfblk63_out1 = dtc_out_6;



  assign cfblk86_out1 = cfblk63_out1 + cfblk47_out1;



  assign cfblk67_out1 = cfblk86_out1 + cfblk67_const_val_1;



  always @(posedge clk or posedge reset)
    begin : cfblk140_process
      if (reset == 1'b1) begin
        cfblk140_reg[0] <= 8'b00000000;
        cfblk140_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk140_reg[0] <= cfblk140_reg_next[0];
          cfblk140_reg[1] <= cfblk140_reg_next[1];
        end
      end
    end

  assign cfblk140_out1 = cfblk140_reg[1];
  assign cfblk140_reg_next[0] = cfblk67_out1;
  assign cfblk140_reg_next[1] = cfblk140_reg[0];



  assign cfblk47_out1 = cfblk140_out1 - cfblk115_out1;



  assign cfblk45_out1 = cfblk47_out1 + cfblk45_const_val_1;



  assign cfblk90_out1 = (cfblk45_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  assign cfblk15_out1 = (cfblk90_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  assign cfblk111_out1 = cfblk15_out1 + cfblk111_const_val_1;



  assign cfblk61_out1 = (cfblk111_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  assign cfblk59_out1 = (cfblk61_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  assign cfblk10_out1 = cfblk59_out1 + cfblk10_const_val_1;



  assign cfblk53_out1 = cfblk35_out1 + cfblk10_out1;



  always @(posedge clk or posedge reset)
    begin : cfblk143_process
      if (reset == 1'b1) begin
        cfblk143_reg[0] <= 8'b00000000;
        cfblk143_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk143_reg[0] <= cfblk143_reg_next[0];
          cfblk143_reg[1] <= cfblk143_reg_next[1];
        end
      end
    end

  assign cfblk143_out1 = cfblk143_reg[1];
  assign cfblk143_reg_next[0] = cfblk53_out1;
  assign cfblk143_reg_next[1] = cfblk143_reg[0];



  assign cfblk13_out1 = cfblk143_out1 + cfblk116_out1;



  assign cfblk68_out1 = cfblk13_out1 + cfblk41_out1;



  assign cfblk122 = cfblk68_out1;

  cfblk130 u_cfblk130 (.In1(cfblk47_out1),  // uint8
                       .Out1(cfblk130_out1)  // uint8
                       );

  assign cfblk22_const_val_1 = 8'b00000000;



  assign cfblk22_out1 = cfblk61_out1 + cfblk22_const_val_1;



  assign cfblk114_out1 = (cfblk22_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  DotProduct u_cfblk104_inst (.in1(cfblk130_out1),  // uint8
                              .in2(cfblk116_out1),  // uint8
                              .out1(cfblk104_out1)  // uint8
                              );

  assign cfblk79_out1 = (cfblk104_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  DotProduct_block u_cfblk105_inst (.in1(cfblk114_out1),  // uint8
                                    .in2(cfblk46_out1),  // uint8
                                    .out1(cfblk105_out1)  // uint8
                                    );

  assign dtc_out_7 = cfblk105_out1;



  assign cfblk56_out1 = dtc_out_7;



  assign cfblk80_out1 = cfblk79_out1 - cfblk56_out1;



  always @(cfblk80_out1, cfblk96_out1) begin
    cfblk62_div_temp = 9'b000000000;
    cfblk62_t_0_0 = 9'b000000000;
    if (cfblk80_out1 == 8'b00000000) begin
      cfblk62_out1 = 8'b11111111;
    end
    else begin
      cfblk62_t_0_0 = {1'b0, cfblk96_out1};
      cfblk62_div_temp = cfblk62_t_0_0 / cfblk80_out1;
      if (cfblk62_div_temp[8] != 1'b0) begin
        cfblk62_out1 = 8'b11111111;
      end
      else begin
        cfblk62_out1 = cfblk62_div_temp[7:0];
      end
    end
  end



  DotProduct_block4 u_cfblk72_inst (.in1(cfblk80_out1),  // uint8
                                    .in2(cfblk106_out1),  // uint8
                                    .out1(cfblk72_out1)  // uint8
                                    );

  Mysubsystem_5 u_Mysubsystem_5 (.In1(cfblk57_out1),  // uint8
                                 .In2(cfblk62_out1),  // uint8
                                 .In3(cfblk72_out1),  // uint8
                                 .cfblk124(Mysubsystem_5_out1)  // uint8
                                 );

  assign cfblk124 = Mysubsystem_5_out1;

  assign Hdl_out = cfblk59_out1;

  assign ce_out = clk_enable;

endmodule  // Subsystem

