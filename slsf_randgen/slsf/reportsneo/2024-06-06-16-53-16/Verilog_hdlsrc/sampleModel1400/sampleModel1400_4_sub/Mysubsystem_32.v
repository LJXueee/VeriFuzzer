// -------------------------------------------------------------
// 
// File Name: D:\ljx\code\GUIDANCE-ljx3\slsf_randgen\slsf\reportsneo\2024-06-06-16-53-16\Verilog_hdlsrc\sampleModel1400\sampleModel1400_4_sub\Mysubsystem_32.v
// Created: 2024-08-12 00:42:32
// 
// Generated by MATLAB 9.14 and HDL Coder 4.1
// 
// -------------------------------------------------------------


// -------------------------------------------------------------
// 
// Module: Mysubsystem_32
// Source Path: sampleModel1400_4_sub/Subsystem/Mysubsystem_32
// Hierarchy Level: 1
// 
// -------------------------------------------------------------

`timescale 1 ns / 1 ns

module Mysubsystem_32
          (In1,
           In2,
           Out1,
           Out2,
           Out3,
           Out4);


  input   [7:0] In1;  // uint8
  input   [7:0] In2;  // uint8
  output  [7:0] Out1;  // uint8
  output  [7:0] Out2;  // uint8
  output  [7:0] Out3;  // uint8
  output  [7:0] Out4;  // uint8


  wire [7:0] cfblk68_out1;  // uint8
  reg [7:0] cfblk141_out1;  // uint8
  reg [8:0] cfblk141_div_temp;  // ufix9
  reg [8:0] cfblk141_t_0_0;  // ufix9


  assign cfblk68_out1 = (In1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  always @(In2, cfblk68_out1) begin
    cfblk141_div_temp = 9'b000000000;
    cfblk141_t_0_0 = 9'b000000000;
    if (cfblk68_out1 == 8'b00000000) begin
      cfblk141_out1 = 8'b11111111;
    end
    else begin
      cfblk141_t_0_0 = {1'b0, In2};
      cfblk141_div_temp = cfblk141_t_0_0 / cfblk68_out1;
      if (cfblk141_div_temp[8] != 1'b0) begin
        cfblk141_out1 = 8'b11111111;
      end
      else begin
        cfblk141_out1 = cfblk141_div_temp[7:0];
      end
    end
  end



  assign Out1 = cfblk141_out1;

  assign Out2 = cfblk68_out1;

  assign Out3 = cfblk141_out1;

  assign Out4 = cfblk141_out1;

endmodule  // Mysubsystem_32

