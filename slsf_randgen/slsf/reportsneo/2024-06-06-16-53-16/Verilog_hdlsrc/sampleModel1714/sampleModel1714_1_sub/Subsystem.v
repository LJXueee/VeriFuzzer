// -------------------------------------------------------------
// 
// File Name: D:\ljx\code\GUIDANCE-ljx3\slsf_randgen\slsf\reportsneo\2024-06-06-16-53-16\Verilog_hdlsrc\sampleModel1714\sampleModel1714_1_sub\Subsystem.v
// Created: 2024-08-13 19:53:08
// 
// Generated by MATLAB 9.14 and HDL Coder 4.1
// 
// 
// -- -------------------------------------------------------------
// -- Rate and Clocking Details
// -- -------------------------------------------------------------
// Model base rate: 1
// Target subsystem base rate: 1
// 
// 
// Clock Enable  Sample Time
// -- -------------------------------------------------------------
// ce_out        1
// -- -------------------------------------------------------------
// 
// 
// Output Signal                 Clock Enable  Sample Time
// -- -------------------------------------------------------------
// Hdl_out                       ce_out        1
// -- -------------------------------------------------------------
// 
// -------------------------------------------------------------


// -------------------------------------------------------------
// 
// Module: Subsystem
// Source Path: sampleModel1714_1_sub/Subsystem
// Hierarchy Level: 0
// 
// -------------------------------------------------------------

`timescale 1 ns / 1 ns

module Subsystem
          (clk,
           reset,
           clk_enable,
           ce_out,
           Hdl_out);


  input   clk;
  input   reset;
  input   clk_enable;
  output  ce_out;
  output  [7:0] Hdl_out;  // uint8


  wire enb;
  wire [7:0] Mysubsystem_43_out2;  // uint8
  wire [7:0] Mysubsystem_19_out1;  // uint8
  wire [7:0] Mysubsystem_19_out2;  // uint8
  wire [7:0] Mysubsystem_41_out1;  // uint8
  wire [7:0] Mysubsystem_41_out2;  // uint8
  wire [7:0] Mysubsystem_43_out1;  // uint8
  wire [7:0] Mysubsystem_43_out3;  // uint8
  wire [7:0] cfblk48_const_val_1;  // uint8
  wire [7:0] cfblk65_out1;  // uint8
  reg [7:0] cfblk194_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk194_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk194_out1;  // uint8
  wire [7:0] cfblk60_out1;  // uint8
  wire [7:0] cfblk32_const_val_1;  // uint8
  wire [7:0] cfblk33_const_val_1;  // uint8
  wire [7:0] dtc_out;  // ufix8
  wire [7:0] cfblk88_out1;  // uint8
  wire [7:0] cfblk152_const_val_1;  // uint8
  wire [7:0] cfblk164_out1;  // uint8
  wire [7:0] cfblk70_out1;  // uint8
  wire [15:0] cfblk163_out1;  // uint16
  wire [7:0] cfblk132_const_val_1;  // uint8
  wire [7:0] cfblk96_const_val_1;  // uint8
  wire [7:0] cfblk58_const_val_1;  // uint8
  wire [7:0] cfblk127_const_val_1;  // uint8
  wire [7:0] cfblk166_out1;  // uint8
  wire [7:0] cfblk117_const_val_1;  // uint8
  wire [7:0] cfblk118_out1;  // uint8
  reg [7:0] cfblk192_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk192_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk192_out1;  // uint8
  wire [7:0] cfblk20_out1;  // uint8
  reg [7:0] cfblk191_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk191_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk191_out1;  // uint8
  wire [7:0] cfblk29_out1;  // uint8
  reg [7:0] cfblk182_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk182_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk182_out1;  // uint8
  reg [7:0] cfblk105_out1;  // uint8
  wire [7:0] cfblk69_out1;  // uint8
  wire [7:0] cfblk19_out1;  // uint8
  wire [7:0] cfblk5_out1;  // uint8
  wire [7:0] dtc_out_1;  // ufix8
  wire [7:0] cfblk157_out1;  // uint8
  reg [7:0] cfblk172_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk172_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk172_out1;  // uint8
  wire [7:0] cfblk93_out1;  // uint8
  reg [7:0] cfblk169_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk169_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk169_out1;  // uint8
  wire [7:0] cfblk148_out1;  // uint8
  wire [7:0] cfblk113_out1;  // uint8
  wire [7:0] cfblk87_out1;  // uint8
  wire [7:0] cfblk34_out1;  // uint8
  wire [7:0] cfblk8_out1;  // uint8
  wire [7:0] cfblk128_out1;  // uint8
  reg [7:0] cfblk171_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk171_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk171_out1;  // uint8
  wire [7:0] cfblk136_out1;  // uint8
  wire [7:0] cfblk127_out1;  // uint8
  wire [7:0] cfblk52_out1;  // uint8
  wire [7:0] dtc_out_2;  // ufix8
  wire [7:0] cfblk68_out1;  // uint8
  wire [7:0] cfblk96_out1;  // uint8
  wire [7:0] cfblk58_out1;  // uint8
  wire [7:0] cfblk107_out1;  // uint8
  wire [7:0] cfblk121_out1;  // uint8
  wire [7:0] cfblk99_out1;  // uint8
  wire [7:0] cfblk119_out1;  // uint8
  wire [7:0] cfblk132_out1;  // uint8
  wire [7:0] cfblk126_out1;  // uint8
  wire [7:0] cfblk80_out1;  // uint8
  wire [7:0] cfblk33_out1;  // uint8
  wire [31:0] cfblk51_add_temp;  // ufix32
  wire [31:0] cfblk51_1;  // ufix32
  wire [31:0] cfblk51_2;  // ufix32
  wire [7:0] cfblk51_out1;  // uint8
  wire [7:0] cfblk94_out1;  // uint8
  reg [7:0] cfblk170_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk170_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk170_out1;  // uint8
  wire [7:0] cfblk154_out1;  // uint8
  wire [7:0] cfblk27_out1;  // uint8
  reg [7:0] cfblk181_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk181_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk181_out1;  // uint8
  wire [7:0] cfblk79_out1;  // uint8
  reg [7:0] cfblk188_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk188_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk188_out1;  // uint8
  wire [7:0] cfblk151_out1;  // uint8
  reg [7:0] cfblk187_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk187_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk187_out1;  // uint8
  wire [7:0] cfblk137_out1;  // uint8
  wire [7:0] cfblk122_out1;  // uint8
  reg [7:0] cfblk175_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk175_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk175_out1;  // uint8
  wire [7:0] cfblk48_out1;  // uint8
  wire [7:0] cfblk24_out1;  // uint8
  wire [7:0] cfblk66_out1;  // uint8
  wire [7:0] cfblk31_out1;  // uint8
  reg [7:0] cfblk176_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk176_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk176_out1;  // uint8
  wire [7:0] cfblk138_out1;  // uint8
  wire [7:0] cfblk152_out1;  // uint8
  wire [7:0] cfblk6_out1;  // uint8
  wire [7:0] cfblk147_out1;  // uint8
  wire [7:0] dtc_out_3;  // ufix8
  wire [7:0] cfblk26_out1;  // uint8
  reg [7:0] cfblk155_out1;  // uint8
  wire [7:0] cfblk36_out1;  // uint8
  wire [7:0] cfblk117_out1;  // uint8
  wire [15:0] Mysubsystem_25_out1;  // uint16
  reg [15:0] cfblk184_reg [0:1];  // ufix16 [2]
  wire [15:0] cfblk184_reg_next [0:1];  // ufix16 [2]
  wire [15:0] cfblk184_out1;  // uint16
  reg [7:0] cfblk174_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk174_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk174_out1;  // uint8
  reg [7:0] cfblk186_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk186_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk186_out1;  // uint8
  wire [7:0] cfblk86_out1;  // uint8
  wire [7:0] cfblk144_out1;  // uint8
  wire [7:0] dtc_out_4;  // ufix8
  wire [7:0] cfblk42_out1;  // uint8
  wire [31:0] cfblk120_add_temp;  // ufix32
  wire [31:0] cfblk120_1;  // ufix32
  wire [31:0] cfblk120_2;  // ufix32
  wire [7:0] cfblk120_out1;  // uint8
  wire [7:0] dtc_out_5;  // ufix8
  wire [7:0] cfblk47_out1;  // uint8
  wire [7:0] cfblk15_out1;  // uint8
  wire [7:0] dtc_out_6;  // ufix8
  wire [7:0] cfblk134_out1;  // uint8
  wire [7:0] cfblk81_out1;  // uint8
  reg [7:0] cfblk185_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk185_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk185_out1;  // uint8
  wire [7:0] cfblk141_out1;  // uint8
  wire [7:0] cfblk95_out1;  // uint8
  reg [7:0] cfblk180_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk180_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk180_out1;  // uint8
  reg [7:0] cfblk149_out1;  // uint8
  wire [7:0] dtc_out_7;  // ufix8
  wire [7:0] cfblk131_out1;  // uint8
  wire [7:0] dtc_out_8;  // ufix8
  wire [7:0] cfblk116_out1;  // uint8
  wire [7:0] cfblk77_out1;  // uint8
  reg [7:0] cfblk190_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk190_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk190_out1;  // uint8
  wire [7:0] cfblk53_out1;  // uint8
  wire [7:0] cfblk102_out1;  // uint8
  wire [7:0] cfblk97_out1;  // uint8
  wire [7:0] cfblk83_out1;  // uint8
  reg [7:0] cfblk76_out1;  // uint8
  wire [7:0] cfblk101_out1;  // uint8
  wire [7:0] cfblk98_out1;  // uint8
  wire [7:0] cfblk167_out1;  // uint8
  wire [7:0] cfblk61_out1;  // uint8
  reg [7:0] cfblk177_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk177_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk177_out1;  // uint8
  wire [7:0] dtc_out_9;  // ufix8
  reg [7:0] cfblk17_out1;  // uint8
  wire [7:0] cfblk153_out1;  // uint8
  wire [7:0] dtc_out_10;  // ufix8
  wire cfblk2_relop1;
  wire [7:0] cfblk195_out1;  // uint8
  wire [7:0] cfblk11_out1;  // uint8
  reg [7:0] cfblk179_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk179_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk179_out1;  // uint8
  wire [7:0] cfblk90_out1;  // uint8
  wire [7:0] cfblk32_out1;  // uint8
  reg [7:0] cfblk109_out1;  // uint8
  wire [7:0] dtc_out_11;  // ufix8
  wire [7:0] cfblk64_out1;  // uint8
  wire [7:0] dtc_out_12;  // ufix8
  wire [7:0] cfblk108_out1;  // uint8
  wire [7:0] cfblk125_out1;  // uint8
  wire [7:0] cfblk3_out1;  // uint8
  wire [7:0] cfblk57_out1;  // uint8
  wire [7:0] cfblk18_out1;  // uint8
  wire [7:0] cfblk4_out1;  // uint8
  wire [7:0] cfblk72_out1;  // uint8
  reg [7:0] cfblk178_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk178_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk178_out1;  // uint8
  wire [7:0] cfblk16_out1;  // uint8
  wire [7:0] cfblk45_out1;  // uint8
  wire [7:0] cfblk142_out1;  // uint8
  wire [7:0] dtc_out_13;  // ufix8
  reg [8:0] cfblk105_div_temp;  // ufix9
  reg [8:0] cfblk105_t_0_0;  // ufix9
  reg [8:0] cfblk155_div_temp;  // ufix9
  reg [8:0] cfblk155_t_0_0;  // ufix9
  reg [8:0] cfblk76_div_temp;  // ufix9
  reg [8:0] cfblk76_t_0_0;  // ufix9
  reg [8:0] cfblk17_div_temp;  // ufix9
  reg [8:0] cfblk17_t_0_0;  // ufix9
  reg [8:0] cfblk109_div_temp;  // ufix9
  reg [8:0] cfblk109_t_0_0;  // ufix9
  reg [8:0] cfblk149_div_temp;  // ufix9
  reg [8:0] cfblk149_t_0_0;  // ufix9


  Mysubsystem_19 u_Mysubsystem_19 (.clk(clk),
                                   .reset(reset),
                                   .enb(clk_enable),
                                   .In1(Mysubsystem_43_out2),  // uint8
                                   .Out1(Mysubsystem_19_out1),  // uint8
                                   .Out2(Mysubsystem_19_out2)  // uint8
                                   );

  Mysubsystem_41 u_Mysubsystem_41 (.In1(Mysubsystem_19_out1),  // uint8
                                   .Out1(Mysubsystem_41_out1),  // uint8
                                   .Out2(Mysubsystem_41_out2)  // uint8
                                   );

  Mysubsystem_43 u_Mysubsystem_43 (.clk(clk),
                                   .reset(reset),
                                   .enb(clk_enable),
                                   .In1(Mysubsystem_41_out1),  // uint8
                                   .Out1(Mysubsystem_43_out1),  // uint8
                                   .Out2(Mysubsystem_43_out2),  // uint8
                                   .Out3(Mysubsystem_43_out3)  // uint8
                                   );

  assign enb = clk_enable;

  assign cfblk48_const_val_1 = 8'b00000000;



  always @(posedge clk or posedge reset)
    begin : cfblk194_process
      if (reset == 1'b1) begin
        cfblk194_reg[0] <= 8'b00000000;
        cfblk194_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk194_reg[0] <= cfblk194_reg_next[0];
          cfblk194_reg[1] <= cfblk194_reg_next[1];
        end
      end
    end

  assign cfblk194_out1 = cfblk194_reg[1];
  assign cfblk194_reg_next[0] = cfblk65_out1;
  assign cfblk194_reg_next[1] = cfblk194_reg[0];



  assign cfblk60_out1 = (cfblk194_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  assign cfblk65_out1 = (cfblk60_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  assign cfblk32_const_val_1 = 8'b00000000;



  assign cfblk33_const_val_1 = 8'b00000000;



  assign dtc_out = Mysubsystem_41_out2;



  assign cfblk88_out1 = dtc_out;



  assign cfblk152_const_val_1 = 8'b00000000;



  assign cfblk164_out1 = 8'b00000001;



  assign cfblk70_out1 = cfblk164_out1 + cfblk65_out1;



  cfblk163 u_cfblk163 (.clk(clk),
                       .reset(reset),
                       .enb(clk_enable),
                       .y(cfblk163_out1)  // uint16
                       );

  assign cfblk132_const_val_1 = 8'b00000000;



  assign cfblk96_const_val_1 = 8'b00000000;



  assign cfblk58_const_val_1 = 8'b00000000;



  assign cfblk127_const_val_1 = 8'b00000000;



  assign cfblk166_out1 = 8'b00000000;



  assign cfblk117_const_val_1 = 8'b00000000;



  always @(posedge clk or posedge reset)
    begin : cfblk192_process
      if (reset == 1'b1) begin
        cfblk192_reg[0] <= 8'b00000000;
        cfblk192_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk192_reg[0] <= cfblk192_reg_next[0];
          cfblk192_reg[1] <= cfblk192_reg_next[1];
        end
      end
    end

  assign cfblk192_out1 = cfblk192_reg[1];
  assign cfblk192_reg_next[0] = cfblk118_out1;
  assign cfblk192_reg_next[1] = cfblk192_reg[0];



  always @(posedge clk or posedge reset)
    begin : cfblk191_process
      if (reset == 1'b1) begin
        cfblk191_reg[0] <= 8'b00000000;
        cfblk191_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk191_reg[0] <= cfblk191_reg_next[0];
          cfblk191_reg[1] <= cfblk191_reg_next[1];
        end
      end
    end

  assign cfblk191_out1 = cfblk191_reg[1];
  assign cfblk191_reg_next[0] = cfblk20_out1;
  assign cfblk191_reg_next[1] = cfblk191_reg[0];



  always @(posedge clk or posedge reset)
    begin : cfblk182_process
      if (reset == 1'b1) begin
        cfblk182_reg[0] <= 8'b00000000;
        cfblk182_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk182_reg[0] <= cfblk182_reg_next[0];
          cfblk182_reg[1] <= cfblk182_reg_next[1];
        end
      end
    end

  assign cfblk182_out1 = cfblk182_reg[1];
  assign cfblk182_reg_next[0] = cfblk29_out1;
  assign cfblk182_reg_next[1] = cfblk182_reg[0];



  assign cfblk19_out1 = cfblk105_out1 + cfblk69_out1;



  cfblk5 u_cfblk5 (.u(cfblk19_out1),  // uint8
                   .y(cfblk5_out1)  // uint8
                   );

  assign dtc_out_1 = cfblk5_out1;



  assign cfblk157_out1 = dtc_out_1;



  always @(posedge clk or posedge reset)
    begin : cfblk172_process
      if (reset == 1'b1) begin
        cfblk172_reg[0] <= 8'b00000000;
        cfblk172_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk172_reg[0] <= cfblk172_reg_next[0];
          cfblk172_reg[1] <= cfblk172_reg_next[1];
        end
      end
    end

  assign cfblk172_out1 = cfblk172_reg[1];
  assign cfblk172_reg_next[0] = cfblk157_out1;
  assign cfblk172_reg_next[1] = cfblk172_reg[0];



  always @(posedge clk or posedge reset)
    begin : cfblk169_process
      if (reset == 1'b1) begin
        cfblk169_reg[0] <= 8'b00000000;
        cfblk169_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk169_reg[0] <= cfblk169_reg_next[0];
          cfblk169_reg[1] <= cfblk169_reg_next[1];
        end
      end
    end

  assign cfblk169_out1 = cfblk169_reg[1];
  assign cfblk169_reg_next[0] = cfblk93_out1;
  assign cfblk169_reg_next[1] = cfblk169_reg[0];



  assign cfblk87_out1 = cfblk148_out1 + cfblk113_out1;



  always @(cfblk166_out1, cfblk87_out1) begin
    cfblk105_div_temp = 9'b000000000;
    cfblk105_t_0_0 = 9'b000000000;
    if (cfblk87_out1 == 8'b00000000) begin
      cfblk105_out1 = 8'b11111111;
    end
    else begin
      cfblk105_t_0_0 = {1'b0, cfblk166_out1};
      cfblk105_div_temp = cfblk105_t_0_0 / cfblk87_out1;
      if (cfblk105_div_temp[8] != 1'b0) begin
        cfblk105_out1 = 8'b11111111;
      end
      else begin
        cfblk105_out1 = cfblk105_div_temp[7:0];
      end
    end
  end



  DotProduct_block1 u_cfblk113_inst (.in1(cfblk60_out1),  // uint8
                                     .in2(cfblk34_out1),  // uint8
                                     .out1(cfblk113_out1)  // uint8
                                     );

  DotProduct_block3 u_cfblk128_inst (.in1(cfblk113_out1),  // uint8
                                     .in2(cfblk8_out1),  // uint8
                                     .out1(cfblk128_out1)  // uint8
                                     );

  always @(posedge clk or posedge reset)
    begin : cfblk171_process
      if (reset == 1'b1) begin
        cfblk171_reg[0] <= 8'b00000000;
        cfblk171_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk171_reg[0] <= cfblk171_reg_next[0];
          cfblk171_reg[1] <= cfblk171_reg_next[1];
        end
      end
    end

  assign cfblk171_out1 = cfblk171_reg[1];
  assign cfblk171_reg_next[0] = cfblk128_out1;
  assign cfblk171_reg_next[1] = cfblk171_reg[0];



  assign cfblk127_out1 = cfblk136_out1 + cfblk127_const_val_1;



  assign dtc_out_2 = cfblk52_out1;



  assign cfblk68_out1 = dtc_out_2;



  assign cfblk96_out1 = cfblk68_out1 + cfblk96_const_val_1;



  assign cfblk58_out1 = cfblk96_out1 + cfblk58_const_val_1;



  assign cfblk107_out1 = cfblk58_out1 - cfblk127_out1;



  assign cfblk121_out1 = cfblk107_out1 + cfblk148_out1;



  assign cfblk99_out1 = (cfblk121_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  assign cfblk34_out1 = cfblk99_out1 + cfblk171_out1;



  assign cfblk132_out1 = cfblk119_out1 + cfblk132_const_val_1;



  DotProduct_block2 u_cfblk126_inst (.in1(cfblk132_out1),  // uint8
                                     .in2(cfblk34_out1),  // uint8
                                     .out1(cfblk126_out1)  // uint8
                                     );

  assign cfblk80_out1 = cfblk126_out1 + cfblk105_out1;



  assign cfblk51_1 = {16'b0, cfblk163_out1};
  assign cfblk51_2 = {24'b0, cfblk33_out1};
  assign cfblk51_add_temp = cfblk51_1 + cfblk51_2;
  assign cfblk51_out1 = cfblk51_add_temp[7:0];



  DotProduct_block6 u_cfblk94_inst (.in1(cfblk51_out1),  // uint8
                                    .in2(cfblk80_out1),  // uint8
                                    .out1(cfblk94_out1)  // uint8
                                    );

  always @(posedge clk or posedge reset)
    begin : cfblk170_process
      if (reset == 1'b1) begin
        cfblk170_reg[0] <= 8'b00000000;
        cfblk170_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk170_reg[0] <= cfblk170_reg_next[0];
          cfblk170_reg[1] <= cfblk170_reg_next[1];
        end
      end
    end

  assign cfblk170_out1 = cfblk170_reg[1];
  assign cfblk170_reg_next[0] = cfblk94_out1;
  assign cfblk170_reg_next[1] = cfblk170_reg[0];



  assign cfblk154_out1 = cfblk170_out1 + cfblk169_out1;



  always @(posedge clk or posedge reset)
    begin : cfblk181_process
      if (reset == 1'b1) begin
        cfblk181_reg[0] <= 8'b00000000;
        cfblk181_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk181_reg[0] <= cfblk181_reg_next[0];
          cfblk181_reg[1] <= cfblk181_reg_next[1];
        end
      end
    end

  assign cfblk181_out1 = cfblk181_reg[1];
  assign cfblk181_reg_next[0] = cfblk27_out1;
  assign cfblk181_reg_next[1] = cfblk181_reg[0];



  always @(posedge clk or posedge reset)
    begin : cfblk188_process
      if (reset == 1'b1) begin
        cfblk188_reg[0] <= 8'b00000000;
        cfblk188_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk188_reg[0] <= cfblk188_reg_next[0];
          cfblk188_reg[1] <= cfblk188_reg_next[1];
        end
      end
    end

  assign cfblk188_out1 = cfblk188_reg[1];
  assign cfblk188_reg_next[0] = cfblk79_out1;
  assign cfblk188_reg_next[1] = cfblk188_reg[0];



  always @(posedge clk or posedge reset)
    begin : cfblk187_process
      if (reset == 1'b1) begin
        cfblk187_reg[0] <= 8'b00000000;
        cfblk187_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk187_reg[0] <= cfblk187_reg_next[0];
          cfblk187_reg[1] <= cfblk187_reg_next[1];
        end
      end
    end

  assign cfblk187_out1 = cfblk187_reg[1];
  assign cfblk187_reg_next[0] = cfblk151_out1;
  assign cfblk187_reg_next[1] = cfblk187_reg[0];



  assign cfblk122_out1 = (cfblk137_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  always @(posedge clk or posedge reset)
    begin : cfblk175_process
      if (reset == 1'b1) begin
        cfblk175_reg[0] <= 8'b00000000;
        cfblk175_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk175_reg[0] <= cfblk175_reg_next[0];
          cfblk175_reg[1] <= cfblk175_reg_next[1];
        end
      end
    end

  assign cfblk175_out1 = cfblk175_reg[1];
  assign cfblk175_reg_next[0] = cfblk122_out1;
  assign cfblk175_reg_next[1] = cfblk175_reg[0];



  assign cfblk24_out1 = cfblk175_out1 + cfblk48_out1;



  assign cfblk8_out1 = (cfblk24_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  assign cfblk66_out1 = cfblk88_out1 + cfblk8_out1;



  always @(posedge clk or posedge reset)
    begin : cfblk176_process
      if (reset == 1'b1) begin
        cfblk176_reg[0] <= 8'b00000000;
        cfblk176_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk176_reg[0] <= cfblk176_reg_next[0];
          cfblk176_reg[1] <= cfblk176_reg_next[1];
        end
      end
    end

  assign cfblk176_out1 = cfblk176_reg[1];
  assign cfblk176_reg_next[0] = cfblk31_out1;
  assign cfblk176_reg_next[1] = cfblk176_reg[0];



  assign cfblk138_out1 = cfblk176_out1 + cfblk66_out1;



  assign cfblk152_out1 = cfblk138_out1 + cfblk152_const_val_1;



  assign cfblk137_out1 = cfblk152_out1 - cfblk187_out1;



  assign cfblk147_out1 = cfblk6_out1 + cfblk137_out1;



  assign dtc_out_3 = cfblk147_out1;



  assign cfblk26_out1 = dtc_out_3;



  always @(cfblk188_out1, cfblk26_out1) begin
    cfblk155_div_temp = 9'b000000000;
    cfblk155_t_0_0 = 9'b000000000;
    if (cfblk188_out1 == 8'b00000000) begin
      cfblk155_out1 = 8'b11111111;
    end
    else begin
      cfblk155_t_0_0 = {1'b0, cfblk26_out1};
      cfblk155_div_temp = cfblk155_t_0_0 / cfblk188_out1;
      if (cfblk155_div_temp[8] != 1'b0) begin
        cfblk155_out1 = 8'b11111111;
      end
      else begin
        cfblk155_out1 = cfblk155_div_temp[7:0];
      end
    end
  end



  Mysubsystem_25 u_Mysubsystem_25 (.In1(cfblk36_out1),  // uint8
                                   .In2(cfblk117_out1),  // uint8
                                   .Out1(Mysubsystem_25_out1)  // uint16
                                   );

  always @(posedge clk or posedge reset)
    begin : cfblk184_process
      if (reset == 1'b1) begin
        cfblk184_reg[0] <= 16'b0000000000000000;
        cfblk184_reg[1] <= 16'b0000000000000000;
      end
      else begin
        if (enb) begin
          cfblk184_reg[0] <= cfblk184_reg_next[0];
          cfblk184_reg[1] <= cfblk184_reg_next[1];
        end
      end
    end

  assign cfblk184_out1 = cfblk184_reg[1];
  assign cfblk184_reg_next[0] = Mysubsystem_25_out1;
  assign cfblk184_reg_next[1] = cfblk184_reg[0];



  always @(posedge clk or posedge reset)
    begin : cfblk174_process
      if (reset == 1'b1) begin
        cfblk174_reg[0] <= 8'b00000000;
        cfblk174_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk174_reg[0] <= cfblk174_reg_next[0];
          cfblk174_reg[1] <= cfblk174_reg_next[1];
        end
      end
    end

  assign cfblk174_out1 = cfblk174_reg[1];
  assign cfblk174_reg_next[0] = cfblk48_out1;
  assign cfblk174_reg_next[1] = cfblk174_reg[0];



  always @(posedge clk or posedge reset)
    begin : cfblk186_process
      if (reset == 1'b1) begin
        cfblk186_reg[0] <= 8'b00000000;
        cfblk186_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk186_reg[0] <= cfblk186_reg_next[0];
          cfblk186_reg[1] <= cfblk186_reg_next[1];
        end
      end
    end

  assign cfblk186_out1 = cfblk186_reg[1];
  assign cfblk186_reg_next[0] = cfblk36_out1;
  assign cfblk186_reg_next[1] = cfblk186_reg[0];



  assign cfblk86_out1 = cfblk20_out1 - cfblk186_out1;



  assign cfblk144_out1 = cfblk86_out1 - cfblk174_out1;



  assign cfblk52_out1 = cfblk117_out1 + cfblk144_out1;



  assign dtc_out_4 = cfblk52_out1;



  assign cfblk42_out1 = dtc_out_4;



  assign cfblk120_1 = {24'b0, cfblk42_out1};
  assign cfblk120_2 = {16'b0, cfblk184_out1};
  assign cfblk120_add_temp = cfblk120_1 + cfblk120_2;
  assign cfblk120_out1 = cfblk120_add_temp[7:0];



  assign dtc_out_5 = cfblk120_out1;



  assign cfblk136_out1 = dtc_out_5;



  assign cfblk47_out1 = (cfblk136_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  assign cfblk15_out1 = cfblk36_out1 - cfblk65_out1;



  assign dtc_out_6 = cfblk15_out1;



  assign cfblk134_out1 = dtc_out_6;



  assign cfblk81_out1 = cfblk151_out1 - cfblk134_out1;



  always @(posedge clk or posedge reset)
    begin : cfblk185_process
      if (reset == 1'b1) begin
        cfblk185_reg[0] <= 8'b00000000;
        cfblk185_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk185_reg[0] <= cfblk185_reg_next[0];
          cfblk185_reg[1] <= cfblk185_reg_next[1];
        end
      end
    end

  assign cfblk185_out1 = cfblk185_reg[1];
  assign cfblk185_reg_next[0] = cfblk81_out1;
  assign cfblk185_reg_next[1] = cfblk185_reg[0];



  assign cfblk36_out1 = cfblk185_out1 - cfblk141_out1;



  assign cfblk27_out1 = cfblk151_out1 + cfblk95_out1;



  always @(posedge clk or posedge reset)
    begin : cfblk180_process
      if (reset == 1'b1) begin
        cfblk180_reg[0] <= 8'b00000000;
        cfblk180_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk180_reg[0] <= cfblk180_reg_next[0];
          cfblk180_reg[1] <= cfblk180_reg_next[1];
        end
      end
    end

  assign cfblk180_out1 = cfblk180_reg[1];
  assign cfblk180_reg_next[0] = cfblk27_out1;
  assign cfblk180_reg_next[1] = cfblk180_reg[0];



  assign dtc_out_7 = cfblk149_out1;



  assign cfblk131_out1 = dtc_out_7;



  assign dtc_out_8 = cfblk131_out1;



  assign cfblk116_out1 = dtc_out_8;



  always @(posedge clk or posedge reset)
    begin : cfblk190_process
      if (reset == 1'b1) begin
        cfblk190_reg[0] <= 8'b00000000;
        cfblk190_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk190_reg[0] <= cfblk190_reg_next[0];
          cfblk190_reg[1] <= cfblk190_reg_next[1];
        end
      end
    end

  assign cfblk190_out1 = cfblk190_reg[1];
  assign cfblk190_reg_next[0] = cfblk77_out1;
  assign cfblk190_reg_next[1] = cfblk190_reg[0];



  DotProduct_block u_cfblk102_inst (.in1(cfblk53_out1),  // uint8
                                    .in2(cfblk190_out1),  // uint8
                                    .out1(cfblk102_out1)  // uint8
                                    );

  assign cfblk77_out1 = cfblk102_out1 + cfblk97_out1;



  assign cfblk69_out1 = cfblk77_out1 + cfblk116_out1;



  assign cfblk48_out1 = cfblk69_out1 + cfblk48_const_val_1;



  always @(cfblk48_out1, cfblk83_out1) begin
    cfblk76_div_temp = 9'b000000000;
    cfblk76_t_0_0 = 9'b000000000;
    if (cfblk48_out1 == 8'b00000000) begin
      cfblk76_out1 = 8'b11111111;
    end
    else begin
      cfblk76_t_0_0 = {1'b0, cfblk83_out1};
      cfblk76_div_temp = cfblk76_t_0_0 / cfblk48_out1;
      if (cfblk76_div_temp[8] != 1'b0) begin
        cfblk76_out1 = 8'b11111111;
      end
      else begin
        cfblk76_out1 = cfblk76_div_temp[7:0];
      end
    end
  end



  assign cfblk101_out1 = (cfblk76_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  cfblk167 u_cfblk167 (.In1(cfblk98_out1),  // uint8
                       .Out1(cfblk167_out1)  // uint8
                       );

  assign cfblk61_out1 = cfblk167_out1 - cfblk101_out1;



  always @(posedge clk or posedge reset)
    begin : cfblk177_process
      if (reset == 1'b1) begin
        cfblk177_reg[0] <= 8'b00000000;
        cfblk177_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk177_reg[0] <= cfblk177_reg_next[0];
          cfblk177_reg[1] <= cfblk177_reg_next[1];
        end
      end
    end

  assign cfblk177_out1 = cfblk177_reg[1];
  assign cfblk177_reg_next[0] = cfblk61_out1;
  assign cfblk177_reg_next[1] = cfblk177_reg[0];



  assign cfblk93_out1 = cfblk177_out1 + cfblk117_out1;



  assign cfblk20_out1 = cfblk93_out1 + cfblk180_out1;



  assign cfblk79_out1 = cfblk20_out1 - cfblk36_out1;



  assign dtc_out_9 = cfblk79_out1;



  assign cfblk151_out1 = dtc_out_9;



  always @(cfblk151_out1, cfblk98_out1) begin
    cfblk17_div_temp = 9'b000000000;
    cfblk17_t_0_0 = 9'b000000000;
    if (cfblk151_out1 == 8'b00000000) begin
      cfblk17_out1 = 8'b11111111;
    end
    else begin
      cfblk17_t_0_0 = {1'b0, cfblk98_out1};
      cfblk17_div_temp = cfblk17_t_0_0 / cfblk151_out1;
      if (cfblk17_div_temp[8] != 1'b0) begin
        cfblk17_out1 = 8'b11111111;
      end
      else begin
        cfblk17_out1 = cfblk17_div_temp[7:0];
      end
    end
  end



  assign cfblk148_out1 = cfblk17_out1 + cfblk153_out1;



  assign dtc_out_10 = cfblk148_out1;



  assign cfblk95_out1 = dtc_out_10;



  assign cfblk2_relop1 = Mysubsystem_19_out2 <= cfblk95_out1;



  assign cfblk195_out1 = {7'b0, cfblk2_relop1};



  assign cfblk11_out1 = (cfblk195_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  always @(posedge clk or posedge reset)
    begin : cfblk179_process
      if (reset == 1'b1) begin
        cfblk179_reg[0] <= 8'b00000000;
        cfblk179_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk179_reg[0] <= cfblk179_reg_next[0];
          cfblk179_reg[1] <= cfblk179_reg_next[1];
        end
      end
    end

  assign cfblk179_out1 = cfblk179_reg[1];
  assign cfblk179_reg_next[0] = cfblk11_out1;
  assign cfblk179_reg_next[1] = cfblk179_reg[0];



  assign cfblk53_out1 = cfblk179_out1 + cfblk47_out1;



  assign cfblk83_out1 = Mysubsystem_43_out1 - cfblk149_out1;



  DotProduct_block5 u_cfblk90_inst (.in1(cfblk83_out1),  // uint8
                                    .in2(cfblk53_out1),  // uint8
                                    .out1(cfblk90_out1)  // uint8
                                    );

  assign cfblk32_out1 = cfblk90_out1 + cfblk32_const_val_1;



  assign cfblk97_out1 = (cfblk32_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  always @(Mysubsystem_43_out3, cfblk97_out1) begin
    cfblk109_div_temp = 9'b000000000;
    cfblk109_t_0_0 = 9'b000000000;
    if (cfblk97_out1 == 8'b00000000) begin
      cfblk109_out1 = 8'b11111111;
    end
    else begin
      cfblk109_t_0_0 = {1'b0, Mysubsystem_43_out3};
      cfblk109_div_temp = cfblk109_t_0_0 / cfblk97_out1;
      if (cfblk109_div_temp[8] != 1'b0) begin
        cfblk109_out1 = 8'b11111111;
      end
      else begin
        cfblk109_out1 = cfblk109_div_temp[7:0];
      end
    end
  end



  assign cfblk33_out1 = cfblk109_out1 + cfblk33_const_val_1;



  assign dtc_out_11 = cfblk33_out1;



  assign cfblk64_out1 = dtc_out_11;



  assign cfblk141_out1 = cfblk64_out1 - cfblk155_out1;



  assign dtc_out_12 = cfblk141_out1;



  assign cfblk31_out1 = dtc_out_12;



  assign cfblk108_out1 = cfblk31_out1 + cfblk181_out1;



  assign cfblk125_out1 = (cfblk108_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  cfblk3 u_cfblk3 (.clk(clk),
                   .reset(reset),
                   .enb(clk_enable),
                   .U(cfblk125_out1),  // uint8
                   .Y(cfblk3_out1)  // uint8
                   );

  assign cfblk57_out1 = cfblk3_out1 + cfblk69_out1;



  DotProduct_block4 u_cfblk18_inst (.in1(cfblk57_out1),  // uint8
                                    .in2(cfblk86_out1),  // uint8
                                    .out1(cfblk18_out1)  // uint8
                                    );

  assign cfblk29_out1 = (cfblk18_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  cfblk4 u_cfblk4 (.clk(clk),
                   .reset(reset),
                   .enb(clk_enable),
                   .U(cfblk29_out1),  // uint8
                   .Y(cfblk4_out1)  // uint8
                   );

  assign cfblk98_out1 = cfblk4_out1 + cfblk70_out1;



  assign cfblk72_out1 = cfblk98_out1 - cfblk86_out1;



  always @(posedge clk or posedge reset)
    begin : cfblk178_process
      if (reset == 1'b1) begin
        cfblk178_reg[0] <= 8'b00000000;
        cfblk178_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk178_reg[0] <= cfblk178_reg_next[0];
          cfblk178_reg[1] <= cfblk178_reg_next[1];
        end
      end
    end

  assign cfblk178_out1 = cfblk178_reg[1];
  assign cfblk178_reg_next[0] = cfblk72_out1;
  assign cfblk178_reg_next[1] = cfblk178_reg[0];



  assign cfblk16_out1 = cfblk178_out1 - cfblk154_out1;



  assign cfblk153_out1 = (cfblk16_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  assign cfblk45_out1 = cfblk153_out1 - cfblk172_out1;



  cfblk6 u_cfblk6 (.clk(clk),
                   .reset(reset),
                   .enb(clk_enable),
                   .U(cfblk45_out1),  // uint8
                   .Y(cfblk6_out1)  // uint8
                   );

  assign cfblk119_out1 = cfblk6_out1 - cfblk182_out1;



  assign cfblk142_out1 = cfblk119_out1 + cfblk191_out1;



  assign cfblk117_out1 = cfblk142_out1 + cfblk117_const_val_1;



  always @(cfblk117_out1, cfblk192_out1) begin
    cfblk149_div_temp = 9'b000000000;
    cfblk149_t_0_0 = 9'b000000000;
    if (cfblk192_out1 == 8'b00000000) begin
      cfblk149_out1 = 8'b11111111;
    end
    else begin
      cfblk149_t_0_0 = {1'b0, cfblk117_out1};
      cfblk149_div_temp = cfblk149_t_0_0 / cfblk192_out1;
      if (cfblk149_div_temp[8] != 1'b0) begin
        cfblk149_out1 = 8'b11111111;
      end
      else begin
        cfblk149_out1 = cfblk149_div_temp[7:0];
      end
    end
  end



  assign dtc_out_13 = cfblk149_out1;



  assign cfblk118_out1 = dtc_out_13;



  assign Hdl_out = cfblk118_out1;

  assign ce_out = clk_enable;

endmodule  // Subsystem

