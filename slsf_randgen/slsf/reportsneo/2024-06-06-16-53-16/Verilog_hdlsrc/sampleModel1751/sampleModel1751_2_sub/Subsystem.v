// -------------------------------------------------------------
// 
// File Name: D:\ljx\code\GUIDANCE-ljx3\slsf_randgen\slsf\reportsneo\2024-06-06-16-53-16\Verilog_hdlsrc\sampleModel1751\sampleModel1751_2_sub\Subsystem.v
// Created: 2024-06-10 12:28:17
// 
// Generated by MATLAB 9.14 and HDL Coder 4.1
// 
// 
// -- -------------------------------------------------------------
// -- Rate and Clocking Details
// -- -------------------------------------------------------------
// Model base rate: 1
// Target subsystem base rate: 1
// 
// 
// Clock Enable  Sample Time
// -- -------------------------------------------------------------
// ce_out        1
// -- -------------------------------------------------------------
// 
// 
// Output Signal                 Clock Enable  Sample Time
// -- -------------------------------------------------------------
// cfblk127                      ce_out        1
// Hdl_out                       ce_out        1
// -- -------------------------------------------------------------
// 
// -------------------------------------------------------------


// -------------------------------------------------------------
// 
// Module: Subsystem
// Source Path: sampleModel1751_2_sub/Subsystem
// Hierarchy Level: 0
// 
// -------------------------------------------------------------

`timescale 1 ns / 1 ns

module Subsystem
          (clk,
           reset,
           clk_enable,
           ce_out,
           cfblk127,
           Hdl_out);


  input   clk;
  input   reset;
  input   clk_enable;
  output  ce_out;
  output  [7:0] cfblk127;  // uint8
  output  [7:0] Hdl_out;  // uint8


  wire enb;
  wire [7:0] cfblk129_out1;  // uint8
  wire [7:0] cfblk128_out1;  // uint8
  wire [7:0] cfblk124_const_val_1;  // uint8
  wire [7:0] cfblk124_out1;  // uint8
  wire [7:0] cfblk48_const_val_1;  // uint8
  wire [7:0] cfblk48_out1;  // uint8
  wire [15:0] cfblk131_out1;  // uint16
  wire [7:0] cfblk115_out1;  // uint8
  wire [7:0] cfblk130_out1;  // uint8
  wire [7:0] dtc_out;  // ufix8
  wire [7:0] cfblk44_out1;  // uint8
  wire [7:0] cfblk4_const_val_1;  // uint8
  wire [7:0] cfblk4_out1;  // uint8
  wire [7:0] cfblk72_const_val_1;  // uint8
  wire [7:0] cfblk118_const_val_1;  // uint8
  wire [7:0] cfblk6_const_val_1;  // uint8
  wire [7:0] cfblk27_const_val_1;  // uint8
  wire [7:0] cfblk110_const_val_1;  // uint8
  wire [7:0] cfblk31_const_val_1;  // uint8
  wire [7:0] cfblk1_out1;  // uint8
  wire [7:0] cfblk5_out1;  // uint8
  wire [7:0] cfblk111_out1;  // uint8
  reg [7:0] cfblk12_out1;  // uint8
  reg [7:0] cfblk36_out1;  // uint8
  wire [7:0] cfblk62_out1;  // uint8
  wire [7:0] cfblk31_out1;  // uint8
  wire [7:0] cfblk84_out1;  // uint8
  wire [7:0] cfblk67_out1;  // uint8
  reg [7:0] cfblk141_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk141_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk141_out1;  // uint8
  wire [7:0] cfblk133_out1;  // uint8
  wire [7:0] dtc_out_1;  // ufix8
  wire [7:0] cfblk32_out1;  // uint8
  wire [7:0] cfblk94_out1;  // uint8
  wire [7:0] cfblk54_out1;  // uint8
  wire [7:0] cfblk83_out1;  // uint8
  wire [7:0] cfblk87_out1;  // uint8
  wire [7:0] dtc_out_2;  // ufix8
  wire [7:0] cfblk92_out1;  // uint8
  wire [7:0] cfblk23_out1;  // uint8
  wire [7:0] cfblk61_out1;  // uint8
  wire [7:0] cfblk134_out1;  // uint8
  wire [7:0] cfblk63_out1;  // uint8
  reg [7:0] cfblk151_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk151_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk151_out1;  // uint8
  reg [7:0] cfblk82_out1;  // uint8
  reg [7:0] cfblk143_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk143_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk143_out1;  // uint8
  wire [7:0] cfblk73_out1;  // uint8
  reg [7:0] cfblk140_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk140_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk140_out1;  // uint8
  wire [7:0] cfblk97_out1;  // uint8
  wire [7:0] cfblk99_out1;  // uint8
  wire [7:0] cfblk52_out1;  // uint8
  reg [7:0] cfblk153_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk153_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk153_out1;  // uint8
  wire [7:0] cfblk100_out1;  // uint8
  wire [7:0] cfblk79_out1;  // uint8
  wire [7:0] dtc_out_3;  // ufix8
  wire [7:0] cfblk108_out1;  // uint8
  wire [7:0] dtc_out_4;  // ufix8
  wire [7:0] cfblk43_out1;  // uint8
  reg [7:0] cfblk70_out1;  // uint8
  wire [7:0] cfblk104_out1;  // uint8
  reg [7:0] cfblk144_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk144_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk144_out1;  // uint8
  wire [7:0] cfblk46_out1;  // uint8
  reg [7:0] cfblk139_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk139_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk139_out1;  // uint8
  reg [7:0] cfblk9_out1;  // uint8
  wire [7:0] cfblk68_out1;  // uint8
  wire [7:0] cfblk27_out1;  // uint8
  wire [7:0] dtc_out_5;  // ufix8
  wire [7:0] dtc_out_6;  // ufix8
  wire [7:0] cfblk38_out1;  // uint8
  reg [7:0] cfblk20_out1;  // uint8
  wire [7:0] cfblk41_out1;  // uint8
  wire [7:0] cfblk132_out1;  // uint8
  wire [7:0] cfblk123_out1;  // uint8
  reg [7:0] cfblk152_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk152_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk152_out1;  // uint8
  wire [7:0] cfblk60_out1;  // uint8
  reg [7:0] cfblk76_out1;  // uint8
  wire [7:0] cfblk74_out1;  // uint8
  reg [7:0] cfblk147_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk147_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk147_out1;  // uint8
  wire [7:0] cfblk110_out1;  // uint8
  wire [7:0] cfblk101_out1;  // uint8
  wire [7:0] cfblk118_out1;  // uint8
  reg [7:0] cfblk145_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk145_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk145_out1;  // uint8
  wire [7:0] cfblk137_out1;  // uint8
  wire [7:0] cfblk122_out1;  // uint8
  wire [7:0] dtc_out_7;  // ufix8
  wire [7:0] cfblk89_out1;  // uint8
  wire [7:0] cfblk107_out1;  // uint8
  wire [7:0] cfblk90_out1;  // uint8
  wire [7:0] cfblk6_out1;  // uint8
  wire [7:0] cfblk86_out1;  // uint8
  wire [31:0] cfblk22_add_temp;  // ufix32
  wire [31:0] cfblk22_1;  // ufix32
  wire [31:0] cfblk22_2;  // ufix32
  wire [7:0] cfblk22_out1;  // uint8
  wire [7:0] cfblk57_out1;  // uint8
  wire [7:0] cfblk96_out1;  // uint8
  reg [7:0] cfblk149_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk149_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk149_out1;  // uint8
  wire [7:0] cfblk42_out1;  // uint8
  reg [7:0] cfblk150_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk150_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk150_out1;  // uint8
  wire [7:0] cfblk69_out1;  // uint8
  wire [7:0] cfblk56_out1;  // uint8
  wire [7:0] cfblk39_out1;  // uint8
  wire [7:0] cfblk35_out1;  // uint8
  wire [7:0] cfblk72_out1;  // uint8
  reg [7:0] cfblk148_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk148_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk148_out1;  // uint8
  reg [7:0] cfblk17_out1;  // uint8
  wire [7:0] cfblk117_out1;  // uint8
  wire [7:0] cfblk13_out1;  // uint8
  wire cfblk3_relop1;
  wire signed [31:0] cfblk80_sub_temp;  // sfix32
  wire signed [31:0] cfblk80_1;  // sfix32
  wire signed [31:0] cfblk80_2;  // sfix32
  wire [7:0] cfblk80_out1;  // uint8
  wire [7:0] cfblk21_out1;  // uint8
  wire [7:0] cfblk24_out1;  // uint8
  wire [7:0] dtc_out_8;  // ufix8
  wire [7:0] cfblk19_out1;  // uint8
  wire [7:0] cfblk29_out1;  // uint8
  wire [7:0] dtc_out_9;  // ufix8
  wire [7:0] cfblk113_out1;  // uint8
  wire [7:0] cfblk34_out1;  // uint8
  reg [7:0] cfblk59_out1;  // uint8
  wire [7:0] cfblk135_out1;  // uint8
  wire [7:0] cfblk136_out1;  // uint8
  reg [7:0] cfblk142_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk142_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk142_out1;  // uint8
  reg [7:0] cfblk146_reg [0:1];  // ufix8 [2]
  wire [7:0] cfblk146_reg_next [0:1];  // ufix8 [2]
  wire [7:0] cfblk146_out1;  // uint8
  wire [7:0] cfblk66_out1;  // uint8
  wire [7:0] cfblk106_out1;  // uint8
  wire [7:0] dtc_out_10;  // ufix8
  wire [7:0] cfblk37_out1;  // uint8
  wire [7:0] cfblk18_const_val_1;  // uint8
  wire [7:0] cfblk18_out1;  // uint8
  reg [8:0] cfblk36_div_temp;  // ufix9
  reg [8:0] cfblk36_t_0_0;  // ufix9
  reg [8:0] cfblk82_div_temp;  // ufix9
  reg [8:0] cfblk82_t_0_0;  // ufix9
  reg [8:0] cfblk9_div_temp;  // ufix9
  reg [8:0] cfblk9_t_0_0;  // ufix9
  reg [8:0] cfblk20_div_temp;  // ufix9
  reg [8:0] cfblk20_t_0_0;  // ufix9
  reg [8:0] cfblk76_div_temp;  // ufix9
  reg [8:0] cfblk76_t_0_0;  // ufix9
  reg [8:0] cfblk17_div_temp;  // ufix9
  reg [8:0] cfblk17_t_0_0;  // ufix9
  reg [8:0] cfblk12_div_temp;  // ufix9
  reg [8:0] cfblk12_t_0_0;  // ufix9
  reg [8:0] cfblk70_div_temp;  // ufix9
  reg [8:0] cfblk70_t_0_0;  // ufix9
  reg [8:0] cfblk59_div_temp;  // ufix9
  reg [8:0] cfblk59_t_0_0;  // ufix9


  assign cfblk129_out1 = 8'b00000001;



  assign cfblk128_out1 = 8'b00000001;



  assign cfblk124_const_val_1 = 8'b00000000;



  assign cfblk124_out1 = cfblk128_out1 + cfblk124_const_val_1;



  assign cfblk48_const_val_1 = 8'b00000000;



  assign cfblk48_out1 = cfblk124_out1 + cfblk48_const_val_1;



  cfblk131 u_cfblk131 (.clk(clk),
                       .reset(reset),
                       .enb(clk_enable),
                       .y(cfblk131_out1)  // uint16
                       );

  assign cfblk115_out1 = cfblk131_out1[7:0];



  assign cfblk130_out1 = 8'b00000001;



  assign dtc_out = cfblk48_out1;



  assign cfblk44_out1 = dtc_out;



  assign cfblk4_const_val_1 = 8'b00000000;



  assign cfblk4_out1 = cfblk44_out1 + cfblk4_const_val_1;



  assign enb = clk_enable;

  assign cfblk72_const_val_1 = 8'b00000000;



  assign cfblk118_const_val_1 = 8'b00000000;



  assign cfblk6_const_val_1 = 8'b00000000;



  assign cfblk27_const_val_1 = 8'b00000000;



  assign cfblk110_const_val_1 = 8'b00000000;



  assign cfblk31_const_val_1 = 8'b00000000;



  DotProduct u_cfblk111_inst (.in1(cfblk1_out1),  // uint8
                              .in2(cfblk5_out1),  // uint8
                              .out1(cfblk111_out1)  // uint8
                              );

  always @(cfblk111_out1, cfblk12_out1) begin
    cfblk36_div_temp = 9'b000000000;
    cfblk36_t_0_0 = 9'b000000000;
    if (cfblk12_out1 == 8'b00000000) begin
      cfblk36_out1 = 8'b11111111;
    end
    else begin
      cfblk36_t_0_0 = {1'b0, cfblk111_out1};
      cfblk36_div_temp = cfblk36_t_0_0 / cfblk12_out1;
      if (cfblk36_div_temp[8] != 1'b0) begin
        cfblk36_out1 = 8'b11111111;
      end
      else begin
        cfblk36_out1 = cfblk36_div_temp[7:0];
      end
    end
  end



  assign cfblk31_out1 = cfblk62_out1 + cfblk31_const_val_1;



  assign cfblk84_out1 = cfblk31_out1 - cfblk1_out1;



  always @(posedge clk or posedge reset)
    begin : cfblk141_process
      if (reset == 1'b1) begin
        cfblk141_reg[0] <= 8'b00000000;
        cfblk141_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk141_reg[0] <= cfblk141_reg_next[0];
          cfblk141_reg[1] <= cfblk141_reg_next[1];
        end
      end
    end

  assign cfblk141_out1 = cfblk141_reg[1];
  assign cfblk141_reg_next[0] = cfblk67_out1;
  assign cfblk141_reg_next[1] = cfblk141_reg[0];



  assign dtc_out_1 = cfblk133_out1;



  assign cfblk32_out1 = dtc_out_1;



  assign cfblk83_out1 = cfblk94_out1 + cfblk54_out1;



  assign dtc_out_2 = cfblk87_out1;



  assign cfblk92_out1 = dtc_out_2;



  DotProduct_block4 u_cfblk61_inst (.in1(cfblk92_out1),  // uint8
                                    .in2(cfblk23_out1),  // uint8
                                    .out1(cfblk61_out1)  // uint8
                                    );

  assign cfblk63_out1 = cfblk134_out1 - cfblk1_out1;



  always @(posedge clk or posedge reset)
    begin : cfblk151_process
      if (reset == 1'b1) begin
        cfblk151_reg[0] <= 8'b00000000;
        cfblk151_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk151_reg[0] <= cfblk151_reg_next[0];
          cfblk151_reg[1] <= cfblk151_reg_next[1];
        end
      end
    end

  assign cfblk151_out1 = cfblk151_reg[1];
  assign cfblk151_reg_next[0] = cfblk63_out1;
  assign cfblk151_reg_next[1] = cfblk151_reg[0];



  always @(posedge clk or posedge reset)
    begin : cfblk143_process
      if (reset == 1'b1) begin
        cfblk143_reg[0] <= 8'b00000000;
        cfblk143_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk143_reg[0] <= cfblk143_reg_next[0];
          cfblk143_reg[1] <= cfblk143_reg_next[1];
        end
      end
    end

  assign cfblk143_out1 = cfblk143_reg[1];
  assign cfblk143_reg_next[0] = cfblk82_out1;
  assign cfblk143_reg_next[1] = cfblk143_reg[0];



  always @(posedge clk or posedge reset)
    begin : cfblk140_process
      if (reset == 1'b1) begin
        cfblk140_reg[0] <= 8'b00000000;
        cfblk140_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk140_reg[0] <= cfblk140_reg_next[0];
          cfblk140_reg[1] <= cfblk140_reg_next[1];
        end
      end
    end

  assign cfblk140_out1 = cfblk140_reg[1];
  assign cfblk140_reg_next[0] = cfblk73_out1;
  assign cfblk140_reg_next[1] = cfblk140_reg[0];



  assign cfblk97_out1 = cfblk140_out1 + cfblk1_out1;



  assign cfblk52_out1 = cfblk130_out1 - cfblk99_out1;



  always @(posedge clk or posedge reset)
    begin : cfblk153_process
      if (reset == 1'b1) begin
        cfblk153_reg[0] <= 8'b00000000;
        cfblk153_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk153_reg[0] <= cfblk153_reg_next[0];
          cfblk153_reg[1] <= cfblk153_reg_next[1];
        end
      end
    end

  assign cfblk153_out1 = cfblk153_reg[1];
  assign cfblk153_reg_next[0] = cfblk52_out1;
  assign cfblk153_reg_next[1] = cfblk153_reg[0];



  assign cfblk100_out1 = cfblk153_out1 + cfblk97_out1;



  assign cfblk87_out1 = cfblk100_out1 + cfblk143_out1;



  assign dtc_out_3 = cfblk79_out1;



  assign cfblk108_out1 = dtc_out_3;



  assign dtc_out_4 = cfblk108_out1;



  assign cfblk43_out1 = dtc_out_4;



  assign cfblk104_out1 = cfblk133_out1 - cfblk70_out1;



  always @(cfblk104_out1, cfblk43_out1) begin
    cfblk82_div_temp = 9'b000000000;
    cfblk82_t_0_0 = 9'b000000000;
    if (cfblk43_out1 == 8'b00000000) begin
      cfblk82_out1 = 8'b11111111;
    end
    else begin
      cfblk82_t_0_0 = {1'b0, cfblk104_out1};
      cfblk82_div_temp = cfblk82_t_0_0 / cfblk43_out1;
      if (cfblk82_div_temp[8] != 1'b0) begin
        cfblk82_out1 = 8'b11111111;
      end
      else begin
        cfblk82_out1 = cfblk82_div_temp[7:0];
      end
    end
  end



  always @(posedge clk or posedge reset)
    begin : cfblk144_process
      if (reset == 1'b1) begin
        cfblk144_reg[0] <= 8'b00000000;
        cfblk144_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk144_reg[0] <= cfblk144_reg_next[0];
          cfblk144_reg[1] <= cfblk144_reg_next[1];
        end
      end
    end

  assign cfblk144_out1 = cfblk144_reg[1];
  assign cfblk144_reg_next[0] = cfblk82_out1;
  assign cfblk144_reg_next[1] = cfblk144_reg[0];



  DotProduct_block5 u_cfblk62_inst (.in1(cfblk46_out1),  // uint8
                                    .in2(cfblk144_out1),  // uint8
                                    .out1(cfblk62_out1)  // uint8
                                    );

  always @(posedge clk or posedge reset)
    begin : cfblk139_process
      if (reset == 1'b1) begin
        cfblk139_reg[0] <= 8'b00000000;
        cfblk139_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk139_reg[0] <= cfblk139_reg_next[0];
          cfblk139_reg[1] <= cfblk139_reg_next[1];
        end
      end
    end

  assign cfblk139_out1 = cfblk139_reg[1];
  assign cfblk139_reg_next[0] = cfblk62_out1;
  assign cfblk139_reg_next[1] = cfblk139_reg[0];



  always @(cfblk139_out1, cfblk87_out1) begin
    cfblk9_div_temp = 9'b000000000;
    cfblk9_t_0_0 = 9'b000000000;
    if (cfblk87_out1 == 8'b00000000) begin
      cfblk9_out1 = 8'b11111111;
    end
    else begin
      cfblk9_t_0_0 = {1'b0, cfblk139_out1};
      cfblk9_div_temp = cfblk9_t_0_0 / cfblk87_out1;
      if (cfblk9_div_temp[8] != 1'b0) begin
        cfblk9_out1 = 8'b11111111;
      end
      else begin
        cfblk9_out1 = cfblk9_div_temp[7:0];
      end
    end
  end



  DotProduct_block6 u_cfblk68_inst (.in1(cfblk9_out1),  // uint8
                                    .in2(cfblk151_out1),  // uint8
                                    .out1(cfblk68_out1)  // uint8
                                    );

  assign cfblk27_out1 = cfblk68_out1 + cfblk27_const_val_1;



  assign dtc_out_5 = cfblk27_out1;



  assign cfblk46_out1 = dtc_out_5;



  assign dtc_out_6 = cfblk46_out1;



  assign cfblk38_out1 = dtc_out_6;



  always @(cfblk38_out1, cfblk62_out1) begin
    cfblk20_div_temp = 9'b000000000;
    cfblk20_t_0_0 = 9'b000000000;
    if (cfblk62_out1 == 8'b00000000) begin
      cfblk20_out1 = 8'b11111111;
    end
    else begin
      cfblk20_t_0_0 = {1'b0, cfblk38_out1};
      cfblk20_div_temp = cfblk20_t_0_0 / cfblk62_out1;
      if (cfblk20_div_temp[8] != 1'b0) begin
        cfblk20_out1 = 8'b11111111;
      end
      else begin
        cfblk20_out1 = cfblk20_div_temp[7:0];
      end
    end
  end



  assign cfblk41_out1 = cfblk1_out1 + cfblk124_out1;



  assign cfblk94_out1 = (cfblk41_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  cfblk132 u_cfblk132 (.In1(cfblk94_out1),  // uint8
                       .Out1(cfblk132_out1)  // uint8
                       );

  assign cfblk54_out1 = cfblk132_out1 - cfblk20_out1;



  assign cfblk99_out1 = (cfblk54_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  always @(posedge clk or posedge reset)
    begin : cfblk152_process
      if (reset == 1'b1) begin
        cfblk152_reg[0] <= 8'b00000000;
        cfblk152_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk152_reg[0] <= cfblk152_reg_next[0];
          cfblk152_reg[1] <= cfblk152_reg_next[1];
        end
      end
    end

  assign cfblk152_out1 = cfblk152_reg[1];
  assign cfblk152_reg_next[0] = cfblk123_out1;
  assign cfblk152_reg_next[1] = cfblk152_reg[0];



  always @(cfblk60_out1, cfblk73_out1) begin
    cfblk76_div_temp = 9'b000000000;
    cfblk76_t_0_0 = 9'b000000000;
    if (cfblk60_out1 == 8'b00000000) begin
      cfblk76_out1 = 8'b11111111;
    end
    else begin
      cfblk76_t_0_0 = {1'b0, cfblk73_out1};
      cfblk76_div_temp = cfblk76_t_0_0 / cfblk60_out1;
      if (cfblk76_div_temp[8] != 1'b0) begin
        cfblk76_out1 = 8'b11111111;
      end
      else begin
        cfblk76_out1 = cfblk76_div_temp[7:0];
      end
    end
  end



  assign cfblk74_out1 = (cfblk76_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  always @(posedge clk or posedge reset)
    begin : cfblk147_process
      if (reset == 1'b1) begin
        cfblk147_reg[0] <= 8'b00000000;
        cfblk147_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk147_reg[0] <= cfblk147_reg_next[0];
          cfblk147_reg[1] <= cfblk147_reg_next[1];
        end
      end
    end

  assign cfblk147_out1 = cfblk147_reg[1];
  assign cfblk147_reg_next[0] = cfblk74_out1;
  assign cfblk147_reg_next[1] = cfblk147_reg[0];



  assign cfblk101_out1 = (cfblk110_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  assign cfblk118_out1 = cfblk101_out1 + cfblk118_const_val_1;



  always @(posedge clk or posedge reset)
    begin : cfblk145_process
      if (reset == 1'b1) begin
        cfblk145_reg[0] <= 8'b00000000;
        cfblk145_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk145_reg[0] <= cfblk145_reg_next[0];
          cfblk145_reg[1] <= cfblk145_reg_next[1];
        end
      end
    end

  assign cfblk145_out1 = cfblk145_reg[1];
  assign cfblk145_reg_next[0] = cfblk118_out1;
  assign cfblk145_reg_next[1] = cfblk145_reg[0];



  assign cfblk122_out1 = cfblk137_out1 + cfblk145_out1;



  assign dtc_out_7 = cfblk122_out1;



  assign cfblk89_out1 = dtc_out_7;



  assign cfblk107_out1 = cfblk89_out1 + cfblk147_out1;



  assign cfblk90_out1 = (cfblk107_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  assign cfblk6_out1 = cfblk90_out1 + cfblk6_const_val_1;



  assign cfblk86_out1 = cfblk6_out1 + cfblk152_out1;



  assign cfblk22_1 = {24'b0, cfblk86_out1};
  assign cfblk22_2 = {16'b0, cfblk131_out1};
  assign cfblk22_add_temp = cfblk22_1 + cfblk22_2;
  assign cfblk22_out1 = cfblk22_add_temp[7:0];



  assign cfblk96_out1 = (cfblk57_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  always @(posedge clk or posedge reset)
    begin : cfblk149_process
      if (reset == 1'b1) begin
        cfblk149_reg[0] <= 8'b00000000;
        cfblk149_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk149_reg[0] <= cfblk149_reg_next[0];
          cfblk149_reg[1] <= cfblk149_reg_next[1];
        end
      end
    end

  assign cfblk149_out1 = cfblk149_reg[1];
  assign cfblk149_reg_next[0] = cfblk96_out1;
  assign cfblk149_reg_next[1] = cfblk149_reg[0];



  cfblk1 u_cfblk1 (.clk(clk),
                   .reset(reset),
                   .enb(clk_enable),
                   .U(cfblk149_out1),  // uint8
                   .Y(cfblk1_out1)  // uint8
                   );

  DotProduct_block3 u_cfblk57_inst (.in1(cfblk4_out1),  // uint8
                                    .in2(cfblk42_out1),  // uint8
                                    .out1(cfblk57_out1)  // uint8
                                    );

  always @(posedge clk or posedge reset)
    begin : cfblk150_process
      if (reset == 1'b1) begin
        cfblk150_reg[0] <= 8'b00000000;
        cfblk150_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk150_reg[0] <= cfblk150_reg_next[0];
          cfblk150_reg[1] <= cfblk150_reg_next[1];
        end
      end
    end

  assign cfblk150_out1 = cfblk150_reg[1];
  assign cfblk150_reg_next[0] = cfblk57_out1;
  assign cfblk150_reg_next[1] = cfblk150_reg[0];



  assign cfblk69_out1 = cfblk130_out1 + cfblk137_out1;



  assign cfblk39_out1 = cfblk69_out1 + cfblk56_out1;



  assign cfblk60_out1 = cfblk39_out1 - cfblk115_out1;



  cfblk133 u_cfblk133 (.In1(cfblk60_out1),  // uint8
                       .Out1(cfblk133_out1)  // uint8
                       );

  assign cfblk35_out1 = cfblk133_out1 + cfblk150_out1;



  assign cfblk42_out1 = cfblk35_out1 + cfblk1_out1;



  assign cfblk72_out1 = cfblk42_out1 + cfblk72_const_val_1;



  always @(posedge clk or posedge reset)
    begin : cfblk148_process
      if (reset == 1'b1) begin
        cfblk148_reg[0] <= 8'b00000000;
        cfblk148_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk148_reg[0] <= cfblk148_reg_next[0];
          cfblk148_reg[1] <= cfblk148_reg_next[1];
        end
      end
    end

  assign cfblk148_out1 = cfblk148_reg[1];
  assign cfblk148_reg_next[0] = cfblk72_out1;
  assign cfblk148_reg_next[1] = cfblk148_reg[0];



  always @(cfblk148_out1, cfblk22_out1) begin
    cfblk17_div_temp = 9'b000000000;
    cfblk17_t_0_0 = 9'b000000000;
    if (cfblk22_out1 == 8'b00000000) begin
      cfblk17_out1 = 8'b11111111;
    end
    else begin
      cfblk17_t_0_0 = {1'b0, cfblk148_out1};
      cfblk17_div_temp = cfblk17_t_0_0 / cfblk22_out1;
      if (cfblk17_div_temp[8] != 1'b0) begin
        cfblk17_out1 = 8'b11111111;
      end
      else begin
        cfblk17_out1 = cfblk17_div_temp[7:0];
      end
    end
  end



  assign cfblk79_out1 = (cfblk117_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  always @(cfblk115_out1, cfblk79_out1) begin
    cfblk12_div_temp = 9'b000000000;
    cfblk12_t_0_0 = 9'b000000000;
    if (cfblk79_out1 == 8'b00000000) begin
      cfblk12_out1 = 8'b11111111;
    end
    else begin
      cfblk12_t_0_0 = {1'b0, cfblk115_out1};
      cfblk12_div_temp = cfblk12_t_0_0 / cfblk79_out1;
      if (cfblk12_div_temp[8] != 1'b0) begin
        cfblk12_out1 = 8'b11111111;
      end
      else begin
        cfblk12_out1 = cfblk12_div_temp[7:0];
      end
    end
  end



  DotProduct_block1 u_cfblk13_inst (.in1(cfblk12_out1),  // uint8
                                    .in2(cfblk17_out1),  // uint8
                                    .out1(cfblk13_out1)  // uint8
                                    );

  assign cfblk5_out1 = cfblk13_out1 + cfblk99_out1;



  assign cfblk3_relop1 = cfblk5_out1 <= cfblk61_out1;



  assign cfblk80_1 = {31'b0, cfblk3_relop1};
  assign cfblk80_2 = {24'b0, cfblk1_out1};
  assign cfblk80_sub_temp = cfblk80_1 - cfblk80_2;
  assign cfblk80_out1 = cfblk80_sub_temp[7:0];



  assign cfblk110_out1 = cfblk80_out1 + cfblk110_const_val_1;



  assign cfblk67_out1 = (cfblk110_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  assign cfblk24_out1 = (cfblk21_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  assign cfblk21_out1 = cfblk134_out1 - cfblk48_out1;



  assign dtc_out_8 = cfblk21_out1;



  assign cfblk19_out1 = dtc_out_8;



  assign cfblk29_out1 = cfblk19_out1 + cfblk24_out1;



  assign dtc_out_9 = cfblk29_out1;



  assign cfblk56_out1 = dtc_out_9;



  assign cfblk113_out1 = cfblk56_out1 - cfblk67_out1;



  always @(cfblk113_out1, cfblk52_out1) begin
    cfblk70_div_temp = 9'b000000000;
    cfblk70_t_0_0 = 9'b000000000;
    if (cfblk52_out1 == 8'b00000000) begin
      cfblk70_out1 = 8'b11111111;
    end
    else begin
      cfblk70_t_0_0 = {1'b0, cfblk113_out1};
      cfblk70_div_temp = cfblk70_t_0_0 / cfblk52_out1;
      if (cfblk70_div_temp[8] != 1'b0) begin
        cfblk70_out1 = 8'b11111111;
      end
      else begin
        cfblk70_out1 = cfblk70_div_temp[7:0];
      end
    end
  end



  DotProduct_block2 u_cfblk34_inst (.in1(cfblk70_out1),  // uint8
                                    .in2(cfblk83_out1),  // uint8
                                    .out1(cfblk34_out1)  // uint8
                                    );

  always @(cfblk32_out1, cfblk34_out1) begin
    cfblk59_div_temp = 9'b000000000;
    cfblk59_t_0_0 = 9'b000000000;
    if (cfblk32_out1 == 8'b00000000) begin
      cfblk59_out1 = 8'b11111111;
    end
    else begin
      cfblk59_t_0_0 = {1'b0, cfblk34_out1};
      cfblk59_div_temp = cfblk59_t_0_0 / cfblk32_out1;
      if (cfblk59_div_temp[8] != 1'b0) begin
        cfblk59_out1 = 8'b11111111;
      end
      else begin
        cfblk59_out1 = cfblk59_div_temp[7:0];
      end
    end
  end



  cfblk135 u_cfblk135 (.In1(cfblk59_out1),  // uint8
                       .Out1(cfblk135_out1)  // uint8
                       );

  cfblk136 u_cfblk136 (.In1(cfblk135_out1),  // uint8
                       .Out1(cfblk136_out1)  // uint8
                       );

  always @(posedge clk or posedge reset)
    begin : cfblk142_process
      if (reset == 1'b1) begin
        cfblk142_reg[0] <= 8'b00000000;
        cfblk142_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk142_reg[0] <= cfblk142_reg_next[0];
          cfblk142_reg[1] <= cfblk142_reg_next[1];
        end
      end
    end

  assign cfblk142_out1 = cfblk142_reg[1];
  assign cfblk142_reg_next[0] = cfblk136_out1;
  assign cfblk142_reg_next[1] = cfblk142_reg[0];



  DotProduct_block7 u_cfblk73_inst (.in1(cfblk142_out1),  // uint8
                                    .in2(cfblk90_out1),  // uint8
                                    .out1(cfblk73_out1)  // uint8
                                    );

  assign cfblk23_out1 = cfblk73_out1 + cfblk141_out1;



  always @(posedge clk or posedge reset)
    begin : cfblk146_process
      if (reset == 1'b1) begin
        cfblk146_reg[0] <= 8'b00000000;
        cfblk146_reg[1] <= 8'b00000000;
      end
      else begin
        if (enb) begin
          cfblk146_reg[0] <= cfblk146_reg_next[0];
          cfblk146_reg[1] <= cfblk146_reg_next[1];
        end
      end
    end

  assign cfblk146_out1 = cfblk146_reg[1];
  assign cfblk146_reg_next[0] = cfblk23_out1;
  assign cfblk146_reg_next[1] = cfblk146_reg[0];



  assign cfblk66_out1 = cfblk146_out1 + cfblk84_out1;



  assign cfblk106_out1 = (cfblk66_out1 > 8'b00000000 ? 8'b00000001 :
              8'b00000000);



  cfblk137 u_cfblk137 (.In1(cfblk106_out1),  // uint8
                       .Out1(cfblk137_out1)  // uint8
                       );

  DotProduct_block u_cfblk117_inst (.in1(cfblk129_out1),  // uint8
                                    .in2(cfblk137_out1),  // uint8
                                    .out1(cfblk117_out1)  // uint8
                                    );

  assign dtc_out_10 = cfblk117_out1;



  assign cfblk37_out1 = dtc_out_10;



  assign cfblk123_out1 = cfblk37_out1 + cfblk36_out1;



  cfblk134 u_cfblk134 (.In1(cfblk123_out1),  // uint8
                       .Out1(cfblk134_out1)  // uint8
                       );

  assign cfblk18_const_val_1 = 8'b00000000;



  assign cfblk18_out1 = cfblk134_out1 + cfblk18_const_val_1;



  assign cfblk127 = cfblk18_out1;

  assign Hdl_out = cfblk92_out1;

  assign ce_out = clk_enable;

endmodule  // Subsystem

