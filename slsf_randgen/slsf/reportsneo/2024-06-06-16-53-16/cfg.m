classdef cfg
    ...
    properties(Constant = true)
        ...
        NUM_TESTS = 1000;                       % Number of random models to generate (and use in differntial testing) 生成模型数，原150
        
        NUM_BLOCKS = [130 170];                 % 块数范围250-300
...
%          phases of CyFuzz
        ...
        LOAD_RNG_STATE = true;                  % -false-Set this `true` if we want to create NEW models each time the script is run. Set to `false` if generating same models at each run of the script is desired. For first time running in a new computer set to false, as this will fail first time if set to true.
        ...
        MAX_HIERARCHY_LEVELS =3;               % Minimum value is 1 indicating a flat model with no hierarchy.层次结构为多少
        ...
        SL_BLOCKLIBS = {
           struct('name', 'Logic and Bit Operations', 'is_blk', false, 'num', 0.04) % 0.1
           %struct('name', 'Discrete', 'is_blk', false, 'num', 0.05)
%             struct('name', 'Continuous', 'is_blk', false,  'num', 0.1)
             struct('name', 'Math Operations', 'is_blk', false,  'num', 0.88) % 0.78
%             struct('name', 'Logic and Bit Operations', 'is_blk', false,  'num', 0.08)
        ...
        SL_BLOCKS_BLACKLIST = {
            ...
            'hdlsllib/Math Operations/Complex to Real-Imag'
            ...
            'hdlsllib/Math Operations/Decrement Real World'
            'hdlsllib/Math Operations/Increment Real World'
